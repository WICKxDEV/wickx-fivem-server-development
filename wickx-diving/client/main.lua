...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('wickx-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    WKXCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('wickx-diving:server:TakeCoral', CurrentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if CurrentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[CurrentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['wickx-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[CurrentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['wickx-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            Zones[k]:onPlayerInOut(function(inside)
                if inside then
                    CurrentArea = k
                    exports['wickx-core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    CurrentArea = 0
...
local function sellCoral()
    ...
    WKXCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ClearPedTasks(playerPed)
        TriggerServerEvent('wickx-diving:server:SellCorals')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['wickx-core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['wickx-core']:HideText()
                end
...
RegisterNetEvent('WKXCore:Client:OnPlayerLoaded', function()
    WKXCore.Functions.TriggerCallback('wickx-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('wickx-diving:client:NewLocations', function()
    WKXCore.Functions.TriggerCallback('wickx-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('wickx-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('wickx-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("wickx-diving:client:SetOxygenLevel", function()
    if OxygenLevel == 0 then
       OxygenLevel = Config.OxygenLevel -- oxygenlevel
       WKXCore.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('wickx-diving:server:removeItemAfterFill')
    else
        WKXCore.Functions.Notify(Lang:t("error.oxygenlevel", {oxygenlevel = OxygenLevel}), 'error')
    end
end)
...
RegisterNetEvent('wickx-diving:client:UseGear', function()
    local ped = PlayerPedId()
    if isWearingSuit == false then
        if OxygenLevel > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped, false) then
                gearAnim()
                WKXCore.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {},
                    function() -- Done
                        deleteGear()
                ...
                WKXCore.Functions.Notify(Lang:t("error.not_standing_up"), 'error')
            end
        else
            WKXCore.Functions.Notify(Lang:t("error.need_otube"), 'error')
        end
    elseif isWearingSuit == true then
        gearAnim()
        WKXCore.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {}, function() -- Done
            SetEnableScuba(ped, false)
            SetPedMaxTimeUnderwater(ped, 50.00)
            CurrentGear.enabled = false
            ClearPedTasks(ped)
            deleteGear()
            WKXCore.Functions.Notify(Lang:t("success.took_out"))
            TriggerServerEvent("InteractSound_SV:PlayOnSource", nil, 0.25)
            isWearingSuit = false
            OxygenLevel = OxygenLevel
        end)
