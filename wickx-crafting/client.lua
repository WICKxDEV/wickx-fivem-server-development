...
local function CraftItem(craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
    WKXCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, reqItem in pairs(requiredItems) do
            ...
            if itemAmount < reqItem.amount then
                ...
                WKXCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')) .. amountToCraft .. 'x ' .. WKXCore.Shared.Items[craftedItem].label, 'error')
                break
            end
        end
        if hasAllMaterials then
            if Config.EnableSkillCheck then
                local success = exports['wickx-minigames']:Skillbar('easy', '12345') -- difficulty and words to enter 
                if success then
                    WKXCore.Functions.Progressbar('crafting_item', 'Crafting ' .. WKXCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                        ...
                        TriggerServerEvent('wickx-crafting:server:receiveItem', craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
                    end)
                else
                    ...
                    TriggerServerEvent('wickx-crafting:server:removeMaterials', randomItem.item, randomAmount)
                    WKXCore.Functions.Notify('Crafting failed, some materials have been lost!', 'error')
                end
            else
                WKXCore.Functions.Progressbar('crafting_item', 'Crafting ' .. WKXCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                    disableMovement = true,
            ...
            WKXCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')), 'error')
        end
    end)
end
...
local function CraftAmount(craftedItem, requiredItems, xpGain, xpType)
    ...
    if dialog and tonumber(dialog.amount) then
        ...
        if amount > 0 then
            ...
            WKXCore.Functions.Notify(string.format(Lang:t('notifications.invalidAmount')), 'error')
        end
    else
        WKXCore.Functions.Notify(string.format(Lang:t('notifications.invalidInput')), 'error')
    end
end
...
local function OpenCraftingMenu(benchType)
    ...
    WKXCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, recipe in pairs(recipes) do
            if currentXP >= recipe.xpRequired then
                ...
                for _, reqItem in pairs(recipe.requiredItems) do
                    ...
                    local itemLabel = WKXCore.Shared.Items[reqItem.item].label
                ...
                local menuItem = {
                    header = WKXCore.Shared.Items[recipe.item].label,
                    txt = itemsText,
                    icon = Config.ImageBasePath .. WKXCore.Shared.Items[recipe.item].image,
                    params = {
                        isAction = true,
                        event = function()
        ...
        exports['wickx-menu']:openMenu(menuItems)
    end)
end
...
local function PickupBench(benchType)
    ...
    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        TriggerServerEvent('wickx-crafting:server:addCraftingTable', benchType)
        WKXCore.Functions.Notify(string.format(Lang:t('notifications.pickupBench')), 'success')
    end
end

-- Events

RegisterNetEvent('wickx-crafting:client:useCraftingTable', function(benchType)
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent('wickx-crafting:server:removeCraftingTable', benchType)
    exports['wickx-target']:AddTargetEntity(workbench, {
        options = {
            {
                icon = 'fas fa-tools',
