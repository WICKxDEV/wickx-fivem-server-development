...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent('wickx-bankrobbery:client:BankSecurity', -1, bankHits, false) end
end
...
RegisterNetEvent('wickx-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == 'paleto' then
        if Config.BigBanks['paleto']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:setBankState', extraInfo = ' (paleto) ', source = source }))
        end
        Config.BigBanks['paleto']['isOpened'] = true
        TriggerEvent('wickx-bankrobbery:server:setTimeout')
    elseif bankId == 'pacific' then
        if Config.BigBanks['pacific']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['coords'][2]) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:setBankState', extraInfo = ' (pacific) ', source = source }))
        end
        ...
        if Config.SmallBanks[bankId]['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:setBankState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['isOpened'] = true
        TriggerEvent('wickx-bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('wickx-bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('wickx-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == 'paleto' or bankId == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:setLockerState', extraInfo = ' (' .. bankId .. ') ', source = source }))
        end
        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:setLockerState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['lockers'][lockerId][state] = bool
    end
    TriggerClientEvent('wickx-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('wickx-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    ...
    if type == 'small' then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:receiveItem', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == 'item' then
                    ...
                    exports['wickx-inventory']:AddItem(src, item.item, itemAmount, false, false, 'wickx-bankrobbery:server:recieveItem')
                    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items[item.item], 'add')
                elseif Config.RewardTypes[itemType].type == 'money' then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    exports['wickx-inventory']:AddItem(src, 'markedbills', math.random(2, 3), false, info, 'wickx-bankrobbery:server:recieveItem')
                    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['wickx-inventory']:AddItem(src, 'security_card_01', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
                TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['security_card_01'], 'add')
            end
        else
            exports['wickx-inventory']:AddItem(src, 'weapon_stungun', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
            TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['weapon_stungun'], 'add')
        end
    elseif type == 'paleto' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:receiveItem', extraInfo = ' (paleto) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == 'money' then
                    ...
                    exports['wickx-inventory']:AddItem(src, 'markedbills', math.random(1, 4), false, info, 'wickx-bankrobbery:server:recieveItem')
                    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['wickx-inventory']:AddItem(src, 'security_card_02', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
                TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['security_card_02'], 'add')
            end
        else
            exports['wickx-inventory']:AddItem(src, 'weapon_vintagepistol', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
            TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['weapon_vintagepistol'], 'add')
        end
    elseif type == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:receiveItem', extraInfo = ' (pacific) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                exports['wickx-inventory']:AddItem(src, 'cryptostick', 1, false, info, 'wickx-bankrobbery:server:recieveItem')
                TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['cryptostick'], 'add')
            end
        else
            local chance = math.random(1, 2)
            local odd = math.random(1, 2)
            if chance == odd then
                exports['wickx-inventory']:AddItem(src, 'weapon_microsmg', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
                TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['weapon_microsmg'], 'add')
            else
                exports['wickx-inventory']:AddItem(src, 'weapon_minismg', 1, false, false, 'wickx-bankrobbery:server:recieveItem')
                TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['weapon_minismg'], 'add')
            end
        end
    end
end)

AddEventHandler('wickx-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('wickx-bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('wickx-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('wickx-bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('wickx-bankrobbery:server:callCops', function(type, bank, coords)
    if type == 'small' then
        if not Config.SmallBanks[bank]['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:callCops', extraInfo = ' (smallbank ' .. bank .. ') ', source = source }))
        end
    elseif type == 'paleto' then
        if not Config.BigBanks['paleto']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:callCops', extraInfo = ' (paleto) ', source = source }))
        end
    elseif type == 'pacific' then
        if not Config.BigBanks['pacific']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'wickx-bankrobbery:server:callCops', extraInfo = ' (pacific) ', source = source }))
        end
    end
    TriggerClientEvent('wickx-bankrobbery:client:robberyCall', -1, type, coords)
end)

RegisterNetEvent('wickx-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent('wickx-bankrobbery:client:SetStationStatus', -1, key, isHit)
    if AllStationsHit() then
        exports['wickx-weathersync']:setBlackout(true)
        TriggerClientEvent('wickx-bankrobbery:client:disableAllBankSecurity', -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports['wickx-weathersync']:setBlackout(false)
                TriggerClientEvent('wickx-bankrobbery:client:enableAllBankSecurity', -1)
...
RegisterNetEvent('wickx-bankrobbery:server:removeElectronicKit', function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['wickx-inventory']:RemoveItem(src, 'electronickit', 1, false, 'wickx-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['electronickit'], 'remove')
    exports['wickx-inventory']:RemoveItem(src, 'trojan_usb', 1, false, 'wickx-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['trojan_usb'], 'remove')
end)

RegisterNetEvent('wickx-bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['wickx-inventory']:RemoveItem(src, 'security_card_' .. number, 1, false, 'wickx-bankrobbery:server:removeBankCard')
    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['security_card_' .. number], 'remove')
end)
...
WKXCore.Functions.CreateCallback('wickx-bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

WKXCore.Functions.CreateCallback('wickx-bankrobbery:server:GetConfig', function(_, cb)
    cb(Config.PowerStations, Config.BigBanks, Config.SmallBanks)
end)

WKXCore.Functions.CreateCallback('thermite:server:check', function(source, cb)
    local Player = WKXCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
    if exports['wickx-inventory']:RemoveItem(source, 'thermite', 1, false, 'thermite:server:check') then
        TriggerClientEvent('wickx-inventory:client:ItemBox', source, WKXCore.Shared.Items['thermite'], 'remove')
        cb(true)
    else
        cb(false)
    end
...
WKXCore.Functions.CreateUseableItem('thermite', function(source)
    local Player = WKXCore.Functions.GetPlayer(source)
    ...
    if Player.Functions.GetItemByName('lighter') then
        ...
        TriggerClientEvent('WKXCore:Notify', source, Lang:t('error.missing_ignition_source'), 'error')
    end
end)

WKXCore.Functions.CreateUseableItem('security_card_01', function(source)
    local Player = WKXCore.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_01') then return end
    TriggerClientEvent('wickx-bankrobbery:UseBankcardA', source)
end)

WKXCore.Functions.CreateUseableItem('security_card_02', function(source)
    local Player = WKXCore.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_02') then return end
    TriggerClientEvent('wickx-bankrobbery:UseBankcardB', source)
end)

WKXCore.Functions.CreateUseableItem('electronickit', function(source)
    local Player = WKXCore.Functions.GetPlayer(source)
