...
local function deliveryPay(source, shop)
    ...
    Player.Functions.AddMoney('bank', Config.DeliveryPrice, 'wickx-shops:deliveryPay')
    if math.random(100) <= 10 then exports['wickx-inventory']:AddItem(source, Config.RewardItem, 1, false, false, 'wickx-shops:deliveryPay') end
end
...
RegisterNetEvent('wickx-shops:server:RestockShopItems', function(shop)
    local src = source
    ...
    TriggerClientEvent('wickx-shops:client:SetShopItems', -1, shop, Config.Locations[shop].products)
end)

RegisterNetEvent('wickx-shops:server:UpdateShopItems', function(shop, itemData, amount) -- called from inventory
...
RegisterNetEvent('wickx-shops:server:DoBail', function(bool)
    local src = source
    ...
    if bool then
        if Player.Functions.RemoveMoney('cash', Config.TruckDeposit, 'tow-received-bail') then
            ...
            TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.paid_with_cash', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('wickx-shops:client:SpawnVehicle', src)
        elseif Player.Functions.RemoveMoney('bank', Config.TruckDeposit, 'tow-received-bail') then
            Bail[Player.PlayerData.citizenid] = Config.TruckDeposit
            TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.paid_with_bank', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('wickx-shops:client:SpawnVehicle', src)
        else
            TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.no_deposit', { value = Config.TruckDeposit }), 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], 'trucker-bail-paid')
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.refund_to_cash', { value = Config.TruckDeposit }), 'success')
        end
    end
end)

RegisterNetEvent('wickx-shops:server:PaySlip', function(drops)
    local src = source
    ...
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.you_earned', { value = payment }), 'success')
end)

-- Opening shops

RegisterNetEvent('wickx-shops:server:openShop', function(data)
    local src = source
    ...
    exports['wickx-inventory']:OpenShop(src, shopName)
end)
