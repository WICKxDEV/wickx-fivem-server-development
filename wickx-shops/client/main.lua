    ...
    CreateThread(function()
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                ...
                TriggerServerEvent('wickx-shops:server:openShop', { shop = currentShop })
                listen = false
                break
            end
            Wait(0)
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        if not v.ped then
            exports['wickx-target']:AddCircleZone(k, vector3(v.coords.x, v.coords.y, v.coords.z), 0.5, {
                name = k,
                debugPoly = false,
                useZ = true
            }, {
                options = {
                    {
                        ...
                        item = v.requiredItem,
                        type = 'server',
                        event = 'wickx-shops:server:openShop',
                        shop = k,
                        job = v.requiredJob,
                        gang = v.requiredGang
                    }
            ...
            if Config.UseTarget then
                exports['wickx-target']:AddTargetEntity(ShopPed[k], {
                    options = {
                        {
                            label = v.targetLabel or defaultTargetLabel,
                            icon = v.targetIcon or defaultTargetIcon,
...
local function tableCheck(inputValue, requiredValue)
    ...
    if shopData.requiredItem then
        itemCheck = exports['wickx-inventory']:HasItem(shopData.requiredItem)
    else
        itemCheck = true
    end
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                if tableCheck(playerData, zone.name) then
                    ...
                    exports['wickx-core']:DrawText(Lang:t('info.open_shop'))
                    listenForControl()
                end
            else
