...
local function setupZones(type, number)
    ...
    elseif type == 'vehicleDeposit' then
        ...
        boxName = 'wickx-shops:vehicleDeposit'
        size = 10
    ...
    if Config.UseTarget and type == 'main' then
        ...
        exports['wickx-target']:AddTargetEntity(deliveryPed, {
            options = {
                {
                    icon = 'fas fa-box',
                    label = 'Start Delivering',
                    action = function()
                        TriggerServerEvent('wickx-shops:server:DoBail', true)
                    end
                }
            },
            distance = 2.0
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == 'main' then
                    TriggerServerEvent('wickx-shops:server:DoBail', true)
                elseif type == 'vehicleDeposit' then
                    TriggerEvent('wickx-shops:client:Vehicle')
                elseif type == 'stores' then
                    markerLocation = coords
                    WKXCore.Functions.Notify(Lang:t('mission.store_reached'))
                    ShowMarker(true)
                    SetDelivering(true)
                end
            else
...
local function getNewLocation()
    ...
    if location ~= 0 then
        ...
        WKXCore.Functions.Notify(Lang:t('success.payslip_time'))
...
local function GetInTrunk()
    ...
    if IsPedInAnyVehicle(ped, false) then
        return WKXCore.Functions.Notify(Lang:t('error.get_out_vehicle'), 'error')
    end
    local pos = GetEntityCoords(ped, true)
    local vehicle = GetVehiclePedIsIn(ped, true)
    local tv = getTruckerVehicle(vehicle)
    if not isTruckerVehicle(vehicle) or CurrentPlate ~= WKXCore.Functions.GetPlate(vehicle) then
        return WKXCore.Functions.Notify(Lang:t('error.vehicle_not_correct'), 'error')
    end
    if not BackDoorsOpen(vehicle) then
        return WKXCore.Functions.Notify(Lang:t('error.backdoors_not_open'), 'error')
    end
    local trunkpos = GetOffsetFromEntityInWorldCoords(vehicle, 0, -2.5, 0)
    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) > Config.Vehicles[tv].trunkpos then
        return WKXCore.Functions.Notify(Lang:t('error.too_far_from_trunk'), 'error')
    end
    if isWorking then return end
    isWorking = true
    WKXCore.Functions.Progressbar('work_carrybox', Lang:t('mission.take_box'), 2000, false, true, {
...
local function Deliver()
    isWorking = true
    Wait(500)
    WKXCore.Functions.Progressbar('work_dropbox', Lang:t('mission.deliver_box'), 2000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        if currentCount == CurrentLocation.dropcount then
            ...
            TriggerServerEvent('wickx-shops:server:RestockShopItems', CurrentLocation.store)
            ...
            if JobsDone == Config.MaxDeliveries then
                WKXCore.Functions.Notify(Lang:t('mission.return_to_station'))
                returnToStation()
            else
                WKXCore.Functions.Notify(Lang:t('mission.goto_next_point'))
                getNewLocation()
            end
        else
            WKXCore.Functions.Notify(Lang:t('mission.another_box'))
        end
    end, function()
        isWorking = false
    end)
...
RegisterNetEvent('wickx-shops:client:SetShopItems', function(shop, shopProducts)
    Config.Locations[shop]['products'] = shopProducts
end)

RegisterNetEvent('wickx-shops:client:SpawnVehicle', function()
    ...
    WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
        ...
        CurrentPlate = WKXCore.Functions.GetPlate(veh)
        getNewLocation()
    end, 'boxville2', coords, true)
end)

RegisterNetEvent('wickx-shops:client:Vehicle', function()
    local ped = PlayerPedId()
    ...
    if inVehicle and isTruckerVehicle(vehicle) then
        if GetPedInVehicleSeat(vehicle, -1) == ped then
            if isTruckerVehicle(vehicle) then
                ...
                TriggerServerEvent('wickx-shops:server:DoBail', false)
                if CurrentBlip ~= nil then
                    RemoveBlip(CurrentBlip)
                    ClearAllBlipRoutes()
                    CurrentBlip = nil
                ...
                if returningToStation or CurrentLocation then
                    ...
                    WKXCore.Functions.Notify(Lang:t('mission.job_completed'), 'success')
                end
            else
                WKXCore.Functions.Notify(Lang:t('error.vehicle_not_correct'), 'error')
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.no_driver'))
        end
    end
end)
            ...
            if IsControlJustReleased(0, 38) then
                if not hasBox then
                    ...
                    if #(GetEntityCoords(PlayerPedId()) - markerLocation) < 5 then
                        ...
                        WKXCore.Functions.Notify(Lang:t('error.too_far_from_delivery'), 'error')
                    end
                end
            end
            sleep = 0
