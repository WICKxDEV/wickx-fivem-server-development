...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        WKXCore.Functions.TriggerCallback('wickx-drugs:server:RequestConfig', function(DealerConfig)
...
local function KnockDoorAnim(home)
    ...
    local myData = WKXCore.Functions.GetPlayerData()
    if home then
        ...
        exports['wickx-core']:DrawText(Lang:t('info.other_dealers_button'), 'left')
        AwaitingInput()
    else
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'knock_door', 0.2)
        Wait(100)
        ...
        TaskPlayAnim(PlayerPed, knockAnimLib, 'exit', 3.0, 3.0, -1, 1, 0, false, false, false)
        Wait(1000)
        WKXCore.Functions.Notify(Lang:t('info.no_one_home'), 'error')
    end
end
...
local function RandomDeliveryItemOnRep()
    local myRep = WKXCore.Functions.GetPlayerData().metadata['rep']['dealer'] or 0
...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        WKXCore.Functions.Notify(Lang:t('info.delivery_search'), 'success')
        if Config.NearbyDeliveries == true then
            ...
            if #nearbyLocations > 0 then
                ...
                WKXCore.Functions.Notify(Lang:t('error.delivery_fail'), 'error')
                if Config.Debug == true then
        ...
        WKXCore.Functions.Notify(Lang:t('info.sending_delivery_email'), 'success')
        TriggerServerEvent('wickx-drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('wickx-phone:server:sendNewMail', {
                sender = Config.Dealers[currentDealer]['name'],
                subject = 'Delivery Location',
                message = Lang:t('info.delivery_info_email', { itemAmount = amount, itemLabel = WKXCore.Shared.Items[waitingDelivery['itemData']['item']]['label'] }),
                button = {
                    enabled = true,
                    buttonEvent = 'wickx-drugs:client:setLocation',
                    buttonData = waitingDelivery
                }
            })
        end)
    else
        WKXCore.Functions.Notify(Lang:t('error.pending_delivery'), 'error')
    end
end
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        PoliceCall()
        WKXCore.Functions.Progressbar('work_dropbox', Lang:t('info.delivering_products'), 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('wickx-drugs:server:successDelivery', activeDelivery, true)
            activeDelivery = nil
            if Config.UseTarget then
                exports['wickx-target']:RemoveZone('drugDeliveryZone')
            else
        ...
        TriggerServerEvent('wickx-drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
local function SetMapBlip(x, y)
    SetNewWaypoint(x, y)
    WKXCore.Functions.Notify(Lang:t('success.route_has_been_set'), 'success');
end
        ...
        while waitingKeyPress do
            ...
            elseif dealerIsHome then
                if IsControlJustPressed(0, 38) then
                    GetClosestDealer()
                    TriggerServerEvent('wickx-drugs:server:dealerShop', currentDealer)
                    exports['wickx-core']:KeyPressed()
...
function InitZones()
    ...
    if Config.UseTarget then
        for k, v in pairs(Config.Dealers) do
            exports['wickx-target']:AddBoxZone('dealer_' .. k, vector3(v.coords.x, v.coords.y, v.coords.z), 1.5, 1.5, {
                name = 'dealer_' .. k,
                heading = v.heading,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        dealerCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dealerIsHome then
                    exports['wickx-core']:DrawText(Lang:t('info.knock_button'), 'left')
                    AwaitingInput()
                elseif dealerIsHome then
                    exports['wickx-core']:DrawText(Lang:t('info.other_dealers_button'), 'left')
                    AwaitingInput()
...
RegisterNetEvent('wickx-drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('wickx-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('wickx-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('wickx-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]['products'][itemData.slot].amount = Config.Dealers[dealer]['products'][itemData.slot].amount - amount
end)

RegisterNetEvent('wickx-drugs:client:setLocation', function(locationData)
    ...
    SetMapBlip(activeDelivery['coords'].x, activeDelivery['coords'].y)
    if Config.UseTarget then
        exports['wickx-target']:AddBoxZone('drugDeliveryZone', vector3(activeDelivery['coords'].x, activeDelivery['coords'].y, activeDelivery['coords'].z), 1.5, 1.5, {
        ...
        drugDeliveryZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                exports['wickx-core']:DrawText(Lang:t('info.deliver_items_button', { itemAmount = activeDelivery['amount'], itemLabel = WKXCore.Shared.Items[activeDelivery['itemData']['item']]['label'] }), 'left')
                CreateThread(function()
...
RegisterNetEvent('wickx-drugs:client:sendDeliveryMail', function(type, deliveryData)
