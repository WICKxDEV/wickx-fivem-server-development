...
local function enterRobberyHouse(house)
    ...
    local data = exports['wickx-interior']:CreateHouseRobbery(coords)
...
local function searchCabin(cabin)
    local ped = PlayerPedId()
    if math.random(1, 100) <= 85 and not WKXCore.Functions.IsWearingGloves() then
    ...
    TriggerServerEvent('wickx-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    FreezeEntityPosition(ped, true)
    IsLockpicking = true
    local success = exports['wickx-minigames']:Skillbar()
    if success then
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('wickx-houserobbery:server:searchFurniture', cabin, currentHouse)
        Config.Houses[currentHouse]['furniture'][cabin]['searched'] = true
        TriggerServerEvent('wickx-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    else
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('wickx-houserobbery:server:SetBusyState', cabin, currentHouse, false)
        WKXCore.Functions.Notify(Lang:t('error.process_cancelled'), 'error', 3500)
        FreezeEntityPosition(ped, false)
        IsLockpicking = false
    end
end
...
RegisterNetEvent('WKXCore:Client:OnPlayerLoaded', function()
    WKXCore.Functions.TriggerCallback('wickx-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('wickx-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('wickx-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('wickx-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]['opened'] = state
end)

RegisterNetEvent('wickx-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]['furniture'][cabin]['searched'] = state
end)

RegisterNetEvent('wickx-houserobbery:client:SetBusyState', function(cabin, house, bool)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    if Config.UseClockHours then
        if GetClockHours() < Config.MinimumTime or GetClockHours() > Config.MaximumTime then
            WKXCore.Functions.Notify(Lang:t('error.not_allowed_time'), 'error', 3500)
            return
        end
    end
        ...
        if CurrentCops >= Config.PoliceOnDutyRequired then
            if not Config.Houses[closestHouse]['opened'] then
                if not usingAdvanced then
                    if Config.RequireScrewdriver and not WKXCore.Functions.HasItem('screwdriverset') then
                        WKXCore.Functions.Notify(Lang:t('error.missing_something'), 'error', 3500)
                        return
                    end
                end
                ...
                if usingAdvanced then
                    local success = exports['wickx-minigames']:Skillbar()
                    if success then
                        TriggerServerEvent('wickx-houserobbery:server:enterHouse', closestHouse)
                        WKXCore.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                            TriggerServerEvent('wickx-houserobbery:server:removeAdvancedLockpick')
                            TriggerEvent('wickx-inventory:client:ItemBox', WKXCore.Shared.Items['advancedlockpick'], 'remove')
                        end
                        WKXCore.Functions.Notify(Lang:t('error.didnt_work'), 'error', 2500)
                    end
                else
                    local success = exports['wickx-minigames']:Skillbar('medium')
                    if success then
                        TriggerServerEvent('wickx-houserobbery:server:enterHouse', closestHouse)
                        WKXCore.Functions.Notify(Lang:t('success.worked'), 'success', 2500)
                    else
                        if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                            TriggerServerEvent('wickx-houserobbery:server:removeLockpick')
                ...
                WKXCore.Functions.Notify(Lang:t('error.door_open'), 'error', 3500)
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.not_enough_police'), 'error', 3500)
        end
    end
end)
...
CreateThread(function()
    Wait(500)
    local requiredItems = {
        [1] = { name = WKXCore.Shared.Items['advancedlockpick']['name'], image = WKXCore.Shared.Items['advancedlockpick']['image'] },
        [2] = { name = WKXCore.Shared.Items['screwdriverset']['name'], image = WKXCore.Shared.Items['screwdriverset']['image'] },
    }
