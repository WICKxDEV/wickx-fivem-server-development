...
local function loadSettings(settings)
    ...
    WKXCore.Functions.Notify(Lang:t('notify.hud_settings_loaded'), 'success')
    Wait(1000)
...
local function restartHud()
    ...
    WKXCore.Functions.Notify(Lang:t('notify.hud_restart'), 'error')
    ...
    Wait(2600)
    WKXCore.Functions.Notify(Lang:t('notify.hud_start'), 'success')
end
...
RegisterNetEvent('hud:client:resetStorage', function()
    ...
    WKXCore.Functions.TriggerCallback('hud:server:getMenu', function(menu)
...
RegisterNetEvent('hud:client:LoadMap', function()
    ...
    if Menu.isToggleMapShapeChecked == 'square' then
        ...
        if Menu.isMapNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.load_square_map'))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.loaded_square_map'))
        end
    elseif Menu.isToggleMapShapeChecked == 'circle' then
        ...
        if Menu.isMapNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.load_circle_map'))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.loaded_circle_map'))
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(_, cb)
    ...
    if Menu.isCinematicModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.cinematic_off'), 'error')
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            WKXCore.Functions.Notify(Lang:t('notify.cinematic_on'))
        end
    end
    TriggerEvent('hud:client:playHudChecklistSound')
    saveSettings()
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in wickx-core
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent('wickx-admin:client:ToggleDevmode', function()
    dev = not dev
end)
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        WKXCore.Functions.Notify(Lang:t('notify.low_fuel'), 'error')
