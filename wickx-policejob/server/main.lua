...
WKXCore.Functions.CreateCallback('police:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = WKXCore.Functions.GetWKXPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == 'leo' and v.PlayerData.job.onduty then
...
WKXCore.Functions.CreateCallback('police:GetCops', function(_, cb)
    local amount = 0
    local players = WKXCore.Functions.GetWKXPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == 'leo' and v.PlayerData.job.onduty then
...
WKXCore.Functions.CreateCallback('police:server:isPlayerDead', function(_, cb, playerId)
    local Player = WKXCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata['isdead'])
end)

WKXCore.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = WKXCore.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(WKXCore.Shared.Weapons[weapon]['name'])
    local retval = false
    if itemInfo then
...
WKXCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = WKXCore.Functions.GetWKXPlayers()
    for _, v in pairs(players) do
...
RegisterNetEvent('wickx-policejob:server:stash', function()
    local src = source
...
RegisterNetEvent('wickx-policejob:server:trash', function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.job.type ~= 'leo' then return end
    exports['wickx-inventory']:OpenInventory(src, 'policetrash', {
        maxweight = 4000000,
        slots = 300,
    })
end)

RegisterNetEvent('wickx-policejob:server:evidence', function(currentEvidence)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.job.type ~= 'leo' then return end
    exports['wickx-inventory']:OpenInventory(src, currentEvidence, {
        maxweight = 4000000,
        slots = 500,
    })
end)
...
RegisterNetEvent('police:server:SetTracker', function(targetId)
    ...
    if not WKXCore.Functions.GetPlayer(src) or not Target then return end

    local TrackerMeta = Target.PlayerData.metadata['tracker']
    if TrackerMeta then
        Target.Functions.SetMetaData('tracker', false)
        TriggerClientEvent('WKXCore:Notify', targetId, Lang:t('success.anklet_taken_off'), 'success')
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.took_anklet_from', { firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname }), 'success')
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData('tracker', true)
        TriggerClientEvent('WKXCore:Notify', targetId, Lang:t('success.put_anklet'), 'success')
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.put_anklet_on', { firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname }), 'success')
...
RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = WKXCore.Functions.GetPlayer(source)
    ...
    TriggerClientEvent('wickx-phone:client:addPoliceAlert', requestId, alertData)
end)
...
WKXCore.Functions.CreateUseableItem('handcuffs', function(source)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateUseableItem('moneybag', function(source, item)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if not exports['wickx-inventory']:RemoveItem(src, 'moneybag', 1, item.slot, 'wickx-policejob:moneybag') then return end
    Player.Functions.AddMoney('cash', tonumber(item.info.cash), 'wickx-policejob:moneybag')
end)
