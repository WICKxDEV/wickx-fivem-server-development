...
AddEventHandler('WKXCore:Client:OnPlayerLoaded', function()
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('wickx-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ['accessory'] = {
...
RegisterNetEvent('WKXCore:Client:SetDuty', function(newDuty)
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    WKXCore.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'police')
