...
local function SetCarItemsInfo()
    ...
    for _, item in pairs(Config.CarItems) do
        local itemInfo = WKXCore.Shared.Items[item.name:lower()]
        if itemInfo then
            items[#items + 1] = {
                name = itemInfo.name,
...
function TakeOutImpound(vehicle)
    ...
    if coords then
        WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
            ...
            WKXCore.Functions.SetVehicleProperties(veh, json.decode(vehicle.mods))
        ...
        WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
            ...
            if Config.VehicleSettings[vehicleInfo] ~= nil then
                if Config.VehicleSettings[vehicleInfo].extras ~= nil then
                    WKXCore.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
                end
...
function MenuGarage(currentSelection)
    ...
    local playerGrade = WKXCore.Functions.GetPlayerData().job.grade.level
...
function MenuImpound(currentSelection)
    ...
    WKXCore.Functions.TriggerCallback('police:GetImpoundedVehicles', function(result)
        local shouldContinue = false
        if result == nil then
            WKXCore.Functions.Notify(Lang:t('error.no_impound'), 'error', 5000)
        else
            shouldContinue = true
            for _, v in pairs(result) do
                local enginePercent = WKXCore.Shared.Round(v.engine / 10, 0)
                local currentFuel = v.fuel
                local vname = WKXCore.Shared.Vehicles[v.vehicle].name
        ...
        if shouldContinue then
            ...
            exports['wickx-menu']:openMenu(impoundMenu)
        end
    end)
end
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            WKXCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                WKXCore.Functions.DeleteVehicle(vehicle)
                TriggerEvent('WKXCore:Notify', Lang:t('success.impounded'), 'success')
                ClearPedTasks(ped)
            end, function() -- Play When Cancel
                ClearPedTasks(ped)
                TriggerEvent('WKXCore:Notify', Lang:t('error.canceled'), 'error')
            end)
        end
    end
end)
    ...
    WKXCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.type == 'leo' then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                WKXCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            WKXCore.Functions.Notify('' .. v .. '')
                        end
                    end
                end, playerId)
            else
                WKXCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function()
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['wickx-input']:ShowInput({
        ...
        if drawer then
            ...
            TriggerServerEvent('wickx-policejob:server:evidence', Lang:t('info.current_evidence', { value = currentEvidence, value2 = drawer.slot }))
        end
    else
        exports['wickx-menu']:closeMenu()
    end
...
RegisterNetEvent('wickx-policejob:ToggleDuty', function()
    TriggerServerEvent('WKXCore:ToggleDuty')
    TriggerServerEvent('police:server:UpdateCurrentCops')
    TriggerServerEvent('police:server:UpdateBlips')
end)

RegisterNetEvent('wickx-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('wickx-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        WKXCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    else
    ...
    CreateThread(function()
        while true do
            ...
            if inStash and PlayerJob.type == 'leo' then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('wickx-policejob:server:stash')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inTrash and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('wickx-policejob:server:trash')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('wickx-police:client:scanFingerPrint')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('wickx-police:client:spawnHelicopter')
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for i = 1, #Config.Locations['duty'] do
            ...
            exports['wickx-target']:AddCircleZone('PoliceDuty_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceDuty_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'client',
                        event = 'wickx-policejob:ToggleDuty',
        ...
        for i = 1, #Config.Locations['stash'] do
            ...
            exports['wickx-target']:AddCircleZone('PoliceStash_' .. i, vector3(v.x, v.y, v.z), 1.0, {
                name = 'PoliceStash_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'server',
                        event = 'wickx-policejob:server:stash',
                        icon = 'fas fa-dungeon',
        ...
        for i = 1, #Config.Locations['trash'] do
            ...
            exports['wickx-target']:AddCircleZone('PoliceTrash_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceTrash_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'server',
                        event = 'wickx-policejob:server:trash',
                        icon = 'fas fa-trash',
        ...
        for i = 1, #Config.Locations['fingerprint'] do
            ...
            exports['wickx-target']:AddCircleZone('PoliceFingerprint_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'wickx-police:client:scanFingerPrint',
        ...
        for i = 1, #Config.Locations['evidence'] do
            local v = Config.Locations['evidence'][i]
            exports['wickx-target']:AddCircleZone('PoliceEvidence_' .. i, vector3(v.x, v.y, v.z), 0.5, {
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['wickx-core']:DrawText(Lang:t('info.on_duty'), 'left')
                dutylistener()
            else
                exports['wickx-core']:DrawText(Lang:t('info.off_duty'), 'left')
                dutylistener()
            end
        else
            dutylisten = false
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['wickx-core']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['wickx-core']:DrawText(Lang:t('info.trash_enter'), 'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['wickx-core']:DrawText(Lang:t('info.scan_fingerprint'), 'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    evidenceCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inEvidence = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['wickx-core']:DrawText(Lang:t('info.evidence_stash_prompt'), 'left')
                evidence()
            end
        else
            inEvidence = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['wickx-core']:HideText()
                    exports['wickx-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['wickx-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['wickx-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['wickx-core']:DrawText(Lang:t('info.store_veh'), 'left')
                    garage()
                else
                    local currentSelection = 0
