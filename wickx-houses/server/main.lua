...
CreateThread(function()
    ...
    TriggerClientEvent('wickx-garages:client:houseGarageConfig', -1, HouseGarages)
    TriggerClientEvent('wickx-houses:client:setHouseConfig', -1, Config.Houses)
end)
...
WKXCore.Commands.Add('decorate', Lang:t('info.decorate_interior'), {}, false, function(source)
    local src = source
    TriggerClientEvent('wickx-houses:client:decorate', src)
end)

WKXCore.Commands.Add('createhouse', Lang:t('info.create_house'), { { name = 'price', help = Lang:t('info.price_of_house') }, { name = 'tier', help = Lang:t('info.tier_number') } }, true, function(source, args)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('wickx-houses:client:createHouses', src, price, tier)
    else
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

WKXCore.Commands.Add('addgarage', Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('wickx-houses:client:addGarage', src)
    else
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

WKXCore.Commands.Add('ring', Lang:t('info.ring_doorbell'), {}, false, function(source)
    local src = source
    TriggerClientEvent('wickx-houses:client:RequestRing', src)
end)

-- Item

WKXCore.Functions.CreateUseableItem('police_stormram', function(source, _)
    local Player = WKXCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty) then
        TriggerClientEvent('wickx-houses:client:HomeInvasion', source)
    else
        TriggerClientEvent('WKXCore:Notify', source, Lang:t('error.emergency_services'), 'error')
    end
end)
...
RegisterNetEvent('wickx-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent('wickx-houses:client:setHouseConfig', src, Config.Houses)
end)

RegisterNetEvent('wickx-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent('wickx-houses:client:createBlip', -1, coords)
end)

RegisterNetEvent('wickx-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", '')
    ...
    TriggerClientEvent('wickx-houses:client:setHouseConfig', -1, Config.Houses)
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('info.added_house', { value = label }))
    TriggerEvent('wickx-log:server:CreateLog', 'house', Lang:t('log.house_created'), 'green', Lang:t('log.house_address', { label = label, price = price, tier = tier, agent = GetPlayerName(src) }))
end)

RegisterNetEvent('wickx-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('wickx-garages:client:addHouseGarage', -1, house, garageInfo)
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('info.added_garage', { value = garageInfo.label }))
end)

RegisterNetEvent('wickx-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('wickx-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('wickx-houses:server:openStash', function(CurrentHouse)
    local src = source
    ...
    if stashSlots and stashWeight then
        exports['wickx-inventory']:OpenInventory(src, CurrentHouse, {
...
RegisterNetEvent('wickx-houses:server:buyHouse', function(house)
    local src = source
    local pData = WKXCore.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.already_owned'), 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('wickx-houses:client:SetClosestHouse', src)
        TriggerClientEvent('wickx-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, 'bought-house') -- 21% Extra house costs
        exports['wickx-banking']:AddMoney('realestate', (HousePrice / 100) * math.random(18, 25), 'House purchase')
        TriggerEvent('wickx-log:server:CreateLog', 'house', Lang:t('log.house_purchased'), 'green', Lang:t('log.house_purchased_by', { house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname }))
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.house_purchased'), 'success', 5000)
    else
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.not_enough_money'), 'error')
    end
end)

RegisterNetEvent('wickx-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('wickx-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('wickx-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('wickx-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('wickx-houses:server:giveKey', function(house, target)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local pData = WKXCore.Functions.GetPlayer(target)
    ...
    if not isHouseOwner(Player.PlayerData.license, Player.PlayerData.citizenid, house) then
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.not_owner'), 'error')
        return
    end
...
RegisterNetEvent('wickx-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.remove_key_from', { firstname = citizenData.firstname, lastname = citizenData.lastname }), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
end)

RegisterNetEvent('wickx-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = WKXCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('wickx-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('wickx-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('wickx-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('wickx-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', { json.encode(decorations), house })
    TriggerClientEvent('wickx-houses:server:sethousedecorations', -1, house, decorations)
end)

RegisterNetEvent('wickx-houses:server:LogoutLocation', function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local tPlayer = WKXCore.Functions.GetPlayer(target)
    ...
    if housekeyholders[house] then
        for _, cid in pairs(housekeyholders[house]) do
            if cid == tPlayer.PlayerData.citizenid then
                TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.already_keys'), 'error', 3500)
                return
            end
        end
        housekeyholders[house][#housekeyholders[house] + 1] = tPlayer.PlayerData.citizenid
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
        TriggerClientEvent('wickx-houses:client:refreshHouse', tPlayer.PlayerData.source)

        TriggerClientEvent('WKXCore:Notify', tPlayer.PlayerData.source, Lang:t('success.recieved_key', { value = Config.Houses[house].adress }), 'success', 2500)
    else
        local sourceTarget = WKXCore.Functions.GetPlayer(src)
        housekeyholders[house] = {
...
RegisterNetEvent('wickx-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('wickx-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('wickx-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('wickx-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('wickx-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateCallback('wickx-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = WKXCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money['bank']
...
WKXCore.Functions.CreateCallback('wickx-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
WKXCore.Functions.CreateCallback('wickx-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
WKXCore.Functions.CreateCallback('wickx-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateCallback('wickx-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

WKXCore.Functions.CreateCallback('wickx-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
WKXCore.Functions.CreateCallback('wickx-phone:server:TransferCid', function(_, cb, NewCid, house)
...
WKXCore.Functions.CreateCallback('wickx-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
WKXCore.Functions.CreateCallback('wickx-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', { house })
    if result[1] then
        retval = result[1]
...
WKXCore.Functions.CreateCallback('wickx-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = WKXCore.Functions.GetPlayer(src)
    if pData then
...
WKXCore.Functions.CreateCallback('wickx-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local MyHouses = {}
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local MyKeys = {}
...
WKXCore.Functions.CreateCallback('wickx-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
