...
local function showEntranceHeaderMenu()
    ...
    if WKXCore.Functions.GetPlayerData().job and WKXCore.Functions.GetPlayerData().job.name == 'realestate' then
        isOwned = true
    end
    ...
    if not isOwned then
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'wickx-houses:client:ViewHouse',
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'wickx-houses:client:EnterHouse',
                    args = {}
                }
            }
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'wickx-houses:client:giveHouseKey',
                    args = {}
                }
            }
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu + 1] = {
                ...
                params = {
                    event = 'wickx-houses:client:RequestRing',
                    args = {}
                }
            }
            headerMenu[#headerMenu + 1] = {
            ...
            if WKXCore.Functions.GetPlayerData().job and WKXCore.Functions.GetPlayerData().job.name == 'police' then
                headerMenu[#headerMenu + 1] = {
                    header = Lang:t('menu.lock_door_police'),
                    params = {
                        event = 'wickx-houses:client:ResetHouse',
                        args = {}
                    }
                }
            end
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        params = {
            event = 'wickx-menu:client:closeMenu'
        }
    }

    if headerMenu and next(headerMenu) then
        exports['wickx-menu']:openMenu(headerMenu)
    end
end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        params = {
            event = 'wickx-houses:client:ExitOwnedHouse',
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'wickx-houses:client:FrontDoorCam',
                args = {}
            }
        }
        headerMenu[#headerMenu + 1] = {
            ...
            params = {
                event = 'wickx-houses:client:AnswerDoorbell',
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['wickx-core']:DrawText(Lang:t('target.open_stash'), 'left')
        else
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['wickx-core']:DrawText(Lang:t('target.outfits'), 'left')
        else
            exports['wickx-core']:HideText()
        end
...
local function RegisterCharactersTarget()
    ...
    charactersTargetBox:onPlayerInOut(function(isPointInside)
        if isPointInside and not entering and isOwned then
            exports['wickx-core']:DrawText(Lang:t('target.change_character'), 'left')
        else
            exports['wickx-core']:HideText()
        end
...
RegisterNetEvent('wickx-houses:client:setHouseConfig', function(houseConfig)
...
local function GetClosestPlayer()
    local closestPlayers = WKXCore.Functions.GetPlayersFromCoords()
...
local function FrontDoorCam(coords)
    ...
    SetCamActive(cam, true)
    RenderScriptCams(true, true, 500, true, true)
    TriggerEvent('wickx-weathersync:client:EnableSync')
    FrontCam = true
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            WKXCore.Functions.TriggerCallback('wickx-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    ...
    TriggerEvent('wickx-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        WKXCore.Functions.TriggerCallback('wickx-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
    if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
        WKXCore.Functions.TriggerCallback('wickx-houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('wickx-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    WKXCore.Functions.TriggerCallback('wickx-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end, ClosestHouse)
...
function HouseKeysMenu()
    ...
    if holders == nil or next(holders) == nil then
        WKXCore.Functions.Notify(Lang:t('error.no_key_holders'), 'error', 3500)
        CloseMenuFull()
    else
        keyholderMenu = {}
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu + 1] = {
                ...
                params = {
                    event = 'wickx-houses:client:OpenClientOptions',
                    args = {
        ...
        exports['wickx-menu']:openMenu(keyholderMenu)
    end
end
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = 'wickx-houses:client:RevokeKey',
                args = {
        ...
        },
        {
            ...
            params = {
                event = 'wickx-houses:client:removeHouseKey',
                args = {}
            }
        },
    }

    exports['wickx-menu']:openMenu(keyholderOptions)
end
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['wickx-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['wickx-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['wickx-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['wickx-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['wickx-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['wickx-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['wickx-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['wickx-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['wickx-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['wickx-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['wickx-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['wickx-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['wickx-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['wickx-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['wickx-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['wickx-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['wickx-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['wickx-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['wickx-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['wickx-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['wickx-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['wickx-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['wickx-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['wickx-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['wickx-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['wickx-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['wickx-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['wickx-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['wickx-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['wickx-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['wickx-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['wickx-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['wickx-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['wickx-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['wickx-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['wickx-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['wickx-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['wickx-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['wickx-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['wickx-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['wickx-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['wickx-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['wickx-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['wickx-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['wickx-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['wickx-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['wickx-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['wickx-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['wickx-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['wickx-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['wickx-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['wickx-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['wickx-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['wickx-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['wickx-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['wickx-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['wickx-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['wickx-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['wickx-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['wickx-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['wickx-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['wickx-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['wickx-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['wickx-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['wickx-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['wickx-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['wickx-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['wickx-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['wickx-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['wickx-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['wickx-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['wickx-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['wickx-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['wickx-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['wickx-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['wickx-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['wickx-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['wickx-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['wickx-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['wickx-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['wickx-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['wickx-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['wickx-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['wickx-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['wickx-interior']:CreateV2Modern3(coords)
    elseif Config.Houses[house].tier == 86 then
        return exports['wickx-interior']:VineWoodHouse1(coords)
    elseif Config.Houses[house].tier == 87 then
        return exports['wickx-interior']:VineWoodHouse2(coords)
    elseif Config.Houses[house].tier == 88 then
        return exports['wickx-interior']:VineWoodHouse3(coords)
    elseif Config.Houses[house].tier == 89 then
        return exports['wickx-interior']:CreateK4GunWarehouse(coords)
    elseif Config.Houses[house].tier == 90 then
        return exports['wickx-interior']:CreateK4LuxuryHouse1(coords)
    elseif Config.Houses[house].tier == 91 then
        return exports['wickx-interior']:CreateK4LuxuryHouse2(coords)
    elseif Config.Houses[house].tier == 92 then
        return exports['wickx-interior']:CreateK4LuxuryHouse3(coords)
    elseif Config.Houses[house].tier == 93 then
        return exports['wickx-interior']:CreateK4LuxuryHouse4(coords)
    elseif Config.Houses[house].tier == 94 then
        return exports['wickx-interior']:CreateK4ManorHouse(coords)
    elseif Config.Houses[house].tier == 95 then
        return exports['wickx-interior']:CreateK4Garage1(coords)
    elseif Config.Houses[house].tier == 96 then
        return exports['wickx-interior']:CreateK4Garage2(coords)
    elseif Config.Houses[house].tier == 97 then
        return exports['wickx-interior']:CreateK4Garage3(coords)
    elseif Config.Houses[house].tier == 98 then
        return exports['wickx-interior']:CreateK4Garage4(coords)
    elseif Config.Houses[house].tier == 99 then
        return exports['wickx-interior']:CreateK4Safehouse(coords)
    elseif Config.Houses[house].tier == 100 then
        return exports['wickx-interior']:CreateK4Warehouse(coords)
    else
        WKXCore.Functions.Notify(Lang:t('error.invalid_tier'), 'error')
    end
end
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('wickx-houses:server:SetInsideMeta', house, true)
    --TriggerEvent('wickx-weathersync:client:DisableSync')
    TriggerEvent('wickx-weathersync:client:EnableSync')
    TriggerEvent('wickx-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
    CloseMenuFull()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        DoScreenFadeOut(250)
        Wait(500)
        exports['wickx-interior']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerEvent('wickx-weathersync:client:EnableSync')
            Wait(250)
            DoScreenFadeIn(250)
            ...
            TriggerEvent('wickx-weed:client:leaveHouse')
            TriggerServerEvent('wickx-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
local function openHouseStash()
    ...
    if CheckDistance(stashLoc, 1.5) then
        ...
        TriggerServerEvent('wickx-houses:server:openStash', CurrentHouse)
    end
end
...
local function changeCharacter()
    ...
    if CheckDistance(logoutLoc, 1.5) then
        ...
        exports['wickx-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('wickx-houses:server:LogoutLocation')
        end)
    end
end
-- Events

RegisterNetEvent('wickx-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('wickx-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('wickx-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('wickx-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('wickx-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('wickx-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('WKXCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('wickx-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('wickx-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('wickx-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerServerEvent('wickx-houses:server:setHouses')
    if ClosestHouse and next(Config.Houses[ClosestHouse].garage) == nil then return end
    TriggerEvent('wickx-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
...
RegisterNetEvent('wickx-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('wickx-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub('%-', ' ')
    TriggerServerEvent('wickx-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('wickx-houses:server:createBlip') end
end)

RegisterNetEvent('wickx-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('wickx-houses:server:addGarage', ClosestHouse, coords)
    else
        WKXCore.Functions.Notify(Lang:t('error.no_house'), 'error')
    end
end)

RegisterNetEvent('wickx-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('wickx-houses:server:lockHouse', false, ClosestHouse)
                WKXCore.Functions.Notify(Lang:t('success.unlocked'), 'success', 2500)
            else
                TriggerServerEvent('wickx-houses:server:lockHouse', true, ClosestHouse)
                WKXCore.Functions.Notify(Lang:t('error.locked'), 'error', 2500)
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.no_keys'), 'error', 3500)
        end
    else
        WKXCore.Functions.Notify(Lang:t('error.no_door'), 'error', 3500)
    end
end)

RegisterNetEvent('wickx-houses:client:RingDoor', function(player, house)
    if ClosestHouse == house and IsInside then
        CurrentDoorBell = player
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
        WKXCore.Functions.Notify(Lang:t('info.door_ringing'))
    end
end)

RegisterNetEvent('wickx-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('wickx-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
            WKXCore.Functions.Notify(Lang:t('error.no_door'), 'error')
        end
    elseif ClosestHouse == nil then
        WKXCore.Functions.Notify(Lang:t('error.no_house'), 'error')
    else
        WKXCore.Functions.Notify(Lang:t('error.no_one_near'), 'error')
    end
end)

RegisterNetEvent('wickx-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            WKXCore.Functions.TriggerCallback('wickx-houses:server:getHouseOwner', function(result)
                if WKXCore.Functions.GetPlayerData().citizenid == result then
                    HouseKeysMenu()
                else
                    WKXCore.Functions.Notify(Lang:t('error.not_owner'), 'error')
                end
            end, ClosestHouse)
        else
            WKXCore.Functions.Notify(Lang:t('error.no_door'), 'error')
...
RegisterNetEvent('wickx-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('wickx-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('wickx-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('wickx-houses:client:enterOwnedHouse', function(house)
    WKXCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('wickx-houses:client:LastLocationHouse', function(houseId)
    WKXCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata['injail'] == 0 then
...
RegisterNetEvent('wickx-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            WKXCore.Functions.TriggerCallback('wickx-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('wickx-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('wickx-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('wickx-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('wickx-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('wickx-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('wickx-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('wickx-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('wickx-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('wickx-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('wickx-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.not_owner'), 'error')
        end
    else
        WKXCore.Functions.Notify(Lang:t('error.not_in_house'), 'error')
    end
end)

RegisterNetEvent('wickx-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('wickx-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    if ClosestHouse ~= nil then
        WKXCore.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
                ...
                if dist < 1 then
                    if Config.Houses[ClosestHouse].locked then
                        if not Config.Houses[ClosestHouse].IsRaming then
                            local success = exports['wickx-minigames']:Skillbar('medium') -- calling like this will just change difficulty and still use 1234
                            if success then
                                DoRamAnimation(true)
                                TriggerServerEvent('wickx-houses:server:lockHouse', false, ClosestHouse)
                                WKXCore.Functions.Notify(Lang:t('success.home_invasion'), 'success')
                                TriggerServerEvent('wickx-houses:server:SetHouseRammed', true, ClosestHouse)
                                TriggerServerEvent('wickx-houses:server:SetRamState', false, ClosestHouse)
                            else
                                TriggerServerEvent('wickx-houses:server:SetRamState', false, ClosestHouse)
                                WKXCore.Functions.Notify(Lang:t('error.failed_invasion'), 'error')
                            end
                            DoRamAnimation(false)
                            TriggerServerEvent('wickx-houses:server:SetRamState', true, ClosestHouse)
                        else
                            WKXCore.Functions.Notify(Lang:t('error.inprogress_invasion'), 'error')
                        end
                    else
                        WKXCore.Functions.Notify(Lang:t('error.already_open'), 'error')
                    end
                else
                    WKXCore.Functions.Notify(Lang:t('error.no_house'), 'error')
                end
            else
                WKXCore.Functions.Notify(Lang:t('error.no_police'), 'error')
            end
        end)
    else
        WKXCore.Functions.Notify(Lang:t('error.no_house'), 'error')
...
RegisterNetEvent('wickx-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('wickx-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('wickx-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('wickx-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('wickx-houses:server:SetRamState', false, ClosestHouse)
        end
        if Config.Houses[ClosestHouse].IsRammed then
            ...
            TriggerServerEvent('wickx-houses:server:lockHouse', true, ClosestHouse)
            RamsDone = 0
            WKXCore.Functions.Notify(Lang:t('success.lock_invasion'), 'success')
        else
            WKXCore.Functions.Notify(Lang:t('error.no_invasion'), 'error')
        end
    end
end)

RegisterNetEvent('wickx-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('wickx-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('wickx-houses:client:AnswerDoorbell', function()
    if not CurrentDoorBell or CurrentDoorBell == 0 then
        WKXCore.Functions.Notify(Lang:t('error.nobody_at_door'))
        return
    end
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent('wickx-houses:server:OpenDoor', CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('wickx-houses:client:ViewHouse', function()
...
RegisterNetEvent('wickx-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('wickx-house:client:RefreshHouseTargets', function()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    WKXCore.Functions.TriggerCallback('wickx-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('wickx-houses:client:refreshBlips') end
    TriggerServerEvent('wickx-houses:server:buyHouse', ClosestHouse)
    cb('ok')
end)
