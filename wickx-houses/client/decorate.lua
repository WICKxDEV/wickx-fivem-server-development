...
local function EnableEditMode()
	...
	TriggerEvent('wickx-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent('wickx-houses:server:savedecorations', ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	exports['wickx-target']:AllowTargeting(true)
	LocalPlayer.state:set('inv_busy', false, true)
	TriggerEvent('wickx-anticheat:client:ToggleDecorate', false)
end
...
--			curSpeed = 1
--		end
--		WKXCore.Functions.Notify(Lang:t('info.speed') .. tostring(speeds[curSpeed]))
--	end
--
...
RegisterNetEvent('wickx-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
			...
			WKXCore.Functions.Notify(Lang:t('error.no_keys'), 'error')
		end
	else
...
RegisterNUICallback('buySelectedObject', function(data, cb)
	WKXCore.Functions.TriggerCallback('wickx-houses:server:buyFurniture', function(isSuccess)
		if isSuccess then
			SetNuiFocus(false, false)
			cursorEnabled = false
			SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				WKXCore.Functions.Notify(Lang:t('error.out_range'), 'error')
			end
		end
	end
end)
