...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "wickx-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['wickx-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        event = "wickx-tow:client:PaySlip"
    ...
    elseif type == "vehicle" then
        event = "wickx-tow:client:Vehicle"
    ...
    elseif type == "towspots" then
        event = "wickx-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        exports['wickx-target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('wickx-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('wickx-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('wickx-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('wickx-tow:client:ShowMarker', true)
                else
                    TriggerEvent('wickx-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
local function deliverVehicle(vehicle)
    ...
    WKXCore.Functions.Notify(Lang:t("mission.delivered_vehicle"), "success")
    WKXCore.Functions.Notify(Lang:t("mission.get_new_vehicle"))
...
RegisterNetEvent('wickx-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if WKXCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            WKXCore.Functions.Notify(Lang:t("error.finish_work"), "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('wickx-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            ...
            if NpcOn and CurrentLocation then
                if GetEntityModel(targetVehicle) ~= joaat(CurrentLocation.model) then
                    WKXCore.Functions.Notify(Lang:t("error.vehicle_not_correct"), "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        WKXCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                WKXCore.Functions.Notify(Lang:t("mission.goto_depot"), "primary", 5000)
                                ...
                                TriggerServerEvent('wickx-tow:server:nano', vehNetID)
                                --remove zone
                                CurrentLocation.zoneCombo:destroy()
                            end
                            WKXCore.Functions.Notify(Lang:t("mission.vehicle_towed"), "success")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                            WKXCore.Functions.Notify(Lang:t("error.failed"), "error")
                        end)
                    end
                end
            end
        else
            WKXCore.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                drawDropOff = false
                WKXCore.Functions.Notify(Lang:t("mission.vehicle_takenoff"), "success")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                WKXCore.Functions.Notify(Lang:t("error.failed"), "error")
            end)
        ...
        WKXCore.Functions.Notify(Lang:t("error.not_towing_vehicle"), "error")
    end
end)

RegisterNetEvent('wickx-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('wickx-tow:server:DoBail', true, vehicleInfo)
        selectedVeh = vehicleInfo
    else
        WKXCore.Functions.Notify(Lang:t("error.too_far_away"), 'error')
    end
end)

RegisterNetEvent('wickx-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('wickx-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('wickx-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("wickx-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
        WKXCore.Functions.Notify(Lang:t("error.no_work_done"), "error")
    end
end)

RegisterNetEvent('wickx-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('wickx-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
