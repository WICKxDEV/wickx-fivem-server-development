...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('wickx-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('wickx-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
...
WKXCore.Commands.Add('setcryptoworth', 'Set crypto value', { { name = 'crypto', help = 'Name of the crypto currency' }, { name = 'Value', help = 'New value of the crypto currency' } }, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('WKXCore:Notify', src, 'You have changed the value of ' .. Crypto.Labels[crypto] .. ' from: $' .. Crypto.Worth[crypto] .. ' to: $' .. NewWorth .. ' (' .. ChangeLabel .. ' ' .. PercentageChange .. '%)')
                Crypto.Worth[crypto] = NewWorth
                TriggerClientEvent('wickx-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
                ...
                TriggerClientEvent('WKXCore:Notify', src, Lang:t('text.you_have_not_given_a_new_value', { crypto = Crypto.Worth[crypto] }))
            end
        else
            TriggerClientEvent('WKXCore:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_wickxbit'))
    end
end, 'admin')

WKXCore.Commands.Add('checkcryptoworth', '', {}, false, function(source)
    local src = source
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('text.the_wickxbit_has_a_value_of', { crypto = Crypto.Worth['wickxbit'] }))
end)

WKXCore.Commands.Add('crypto', '', {}, false, function(source)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth['wickxbit'])

    TriggerClientEvent('WKXCore:Notify', src, Lang:t('text.you_have_with_a_value_of', { playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto, mypocket = MyPocket }))
end)

-- Events

RegisterServerEvent('wickx-crypto:server:FetchWorth', function()
    for name, _ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('wickx-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('wickx-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('wickx-crypto:server:ExchangeFail', function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        exports['wickx-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'wickx-crypto:server:ExchangeFail')
        TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('wickx-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('wickx-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('wickx-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('wickx-crypto:server:SyncReboot', function()
    TriggerClientEvent('wickx-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('wickx-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        ...
        exports['wickx-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'wickx-crypto:server:ExchangeSuccess')
        Player.Functions.AddMoney('crypto', Amount, 'wickx-crypto:server:ExchangeSuccess')
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for', { amount = Amount }), 'success', 3500)
        TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('wickx-phone:client:AddTransaction', src, Player, {}, Lang:t('credit.there_are_amount_credited', { amount = Amount }), 'Credit')
    end
end)

-- Callbacks

WKXCore.Functions.CreateCallback('wickx-crypto:server:HasSticky', function(source, cb)
...
WKXCore.Functions.CreateCallback('wickx-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = WKXCore.Functions.GetPlayer(source)
    local CryptoData = {
...
WKXCore.Functions.CreateCallback('wickx-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = WKXCore.Functions.GetPlayer(source)
    local total_price = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['wickxbit']))
    if Player.PlayerData.money.bank >= total_price then
        local CryptoData = {
            History = Crypto.History['wickxbit'],
            Worth = Crypto.Worth['wickxbit'],
            Portfolio = Player.PlayerData.money.crypto + tonumber(data.Coins),
            WalletId = Player.PlayerData.metadata['walletid'],
        }
        Player.Functions.RemoveMoney('bank', total_price, 'bought crypto')
        TriggerClientEvent('wickx-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_wickxbit_purchased', { dataCoins = tonumber(data.Coins) }), 'Credit')
...
WKXCore.Functions.CreateCallback('wickx-crypto:server:SellCrypto', function(source, cb, data)
    local Player = WKXCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        local amount = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['wickxbit']))
        TriggerClientEvent('wickx-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold', { dataCoins = tonumber(data.Coins) }), 'Debit')
...
WKXCore.Functions.CreateCallback('wickx-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('wickx-phone:client:AddTransaction', source, Player, data, 'You have ' .. tonumber(data.Coins) .. " WKXBit('s) transferred!", 'Debit')
            local Target = WKXCore.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins), 'transfer crypto')
                TriggerClientEvent('wickx-phone:client:AddTransaction', Target.PlayerData.source, Player, data, 'There are ' .. tonumber(data.Coins) .. " WKXBit('s) credited!", 'Credit')
            else
    ...
    Citizen.CreateThread(function()
        ...
        while (true) do
            ...
            if type(get_coin_price) == 'number' then
                Crypto.Worth['wickxbit'] = get_coin_price
            else
