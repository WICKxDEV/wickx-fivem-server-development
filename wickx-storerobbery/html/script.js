...
Padlock.Close = function () {
    ...
    $.post("https://wickx-storerobbery/PadLockClose");
};
...
Keypad.Close = function (data) {
    ...
    $.post("https://wickx-storerobbery/PadLockClose");
    if (data.error != null) {
        $.post("https://wickx-storerobbery/CombinationFail");
    }
};
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post(
        "https://wickx-storerobbery/TryCombination",
        JSON.stringify({
            combination: e.value,
        })
    );
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle && angle < (comboArr[i] + numOffset) * tickAngle) {
                ...
                // on unlock
                $.post("https://wickx-storerobbery/callcops");
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post("https://wickx-storerobbery/PadLockSuccess");
                    Padlock.Close();
                }
            }
            break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            if (CurrentType == "keypad") {
                ...
                $.post("https://wickx-storerobbery/exit");
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post("https://wickx-storerobbery/fail");
    setTimeout(function () {
        reset();
    }, 250);
}
...
function unlock() {
    gameOver = true;
    $.post("https://wickx-storerobbery/success");
    solveDeg = Math.random() * 180 - 90;
    solvePadding = 4;
    maxDistFromSolve = 45;
    pinRot = 1;
