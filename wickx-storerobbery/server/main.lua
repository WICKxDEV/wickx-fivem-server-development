...
RegisterNetEvent('wickx-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
    ...
    if isDone then
        ...
        exports['wickx-inventory']:AddItem(src, 'markedbills', bags, false, info, 'wickx-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            exports['wickx-inventory']:AddItem(src, 'stickynote', 1, false, info, 'wickx-storerobbery:server:takeMoney')
            TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['stickynote'], 'add')
        end
    end
end)

RegisterNetEvent('wickx-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('wickx-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('wickx-storerobbery:server:setSafeStatus', function(safe)
    Config.Safes[safe].robbed = true
    TriggerClientEvent('wickx-storerobbery:client:setSafeStatus', -1, safe, true)

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        Config.Safes[safe].robbed = false
        TriggerClientEvent('wickx-storerobbery:client:setSafeStatus', -1, safe, false)
    end)
end)

RegisterNetEvent('wickx-storerobbery:server:SafeReward', function(safe)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    ...
    exports['wickx-inventory']:AddItem(src, 'markedbills', bags, false, info, 'wickx-storerobbery:server:SafeReward')
    TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['markedbills'], 'add')
    local luck = math.random(1, 100)
    local odd = math.random(1, 100)
    if luck <= 10 then
        exports['wickx-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'wickx-storerobbery:server:SafeReward')
        TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['wickx-inventory']:AddItem(src, 'goldbar', 1, false, false, 'wickx-storerobbery:server:SafeReward')
            TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['goldbar'], 'add')
        end
    end
end)

RegisterNetEvent('wickx-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == 'safe' then
    ...
    TriggerClientEvent('wickx-storerobbery:client:robberyCall', -1, type, safe, streetLabel, coords)
    TriggerClientEvent('wickx-phone:client:addPoliceAlert', -1, alertData)
end)

RegisterNetEvent('wickx-storerobbery:server:removeAdvancedLockpick', function()
    local Player = WKXCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['wickx-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'wickx-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('wickx-storerobbery:server:removeLockpick', function()
    local Player = WKXCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['wickx-inventory']:RemoveItem(source, 'lockpick', 1, false, 'wickx-storerobbery:server:removeLockpick')
end)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('wickx-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
WKXCore.Functions.CreateCallback('wickx-storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

WKXCore.Functions.CreateCallback('wickx-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

WKXCore.Functions.CreateCallback('wickx-storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

WKXCore.Functions.CreateCallback('wickx-storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)
