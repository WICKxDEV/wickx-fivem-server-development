...
RegisterServerEvent("wickx-clothing:saveSkin", function(model, skin)
    local src = source
...
RegisterServerEvent("wickx-clothes:loadPlayerSkin", function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', { Player.PlayerData.citizenid, 1 })
    if result[1] ~= nil then
        TriggerClientEvent("wickx-clothes:loadSkin", src, false, result[1].model, result[1].skin)
    else
        TriggerClientEvent("wickx-clothes:loadSkin", src, true)
    end
end)

RegisterServerEvent("wickx-clothes:saveOutfit", function(outfitName, model, skinData)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if model ~= nil and skinData ~= nil then
        ...
        MySQL.insert('INSERT INTO player_outfits (citizenid, outfitname, model, skin, outfitId) VALUES (?, ?, ?, ?, ?)', {
            ...
            if result[1] ~= nil then
                TriggerClientEvent('wickx-clothing:client:reloadOutfits', src, result)
            else
                TriggerClientEvent('wickx-clothing:client:reloadOutfits', src, nil)
            end
        end)
    end
end)

RegisterServerEvent("wickx-clothing:server:removeOutfit", function(outfitName, outfitId)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateCallback('wickx-clothing:server:getOutfits', function(source, cb)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local anusVal = {}
