...
WKXClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://wickx-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://wickx-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://wickx-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function(model) {
                    $("#current-model").html("<p>" + model + "</p>")
                });
                canChange = true;
                WKXClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    WKXClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function() {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://wickx-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://wickx-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://wickx-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://wickx-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
WKXClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        ...
        switch (event.data.action) {
            case "open":
                WKXClothing.Open(event.data);
                break;
            case "close":
                WKXClothing.Close();
                break;
            case "updateMax":
                WKXClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                WKXClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                WKXClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                WKXClothing.ResetValues();
                break;
...
WKXClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e) {
    e.preventDefault();
    WKXClothing.Close();
    $.post('https://wickx-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e) {
    e.preventDefault();
    WKXClothing.Close();
    $.post('https://wickx-clothing/resetOutfit');
});

WKXClothing.SetCurrentValues = function(clothingValues) {
...
WKXClothing.Open = function(data) {
    ...
    WKXClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    WKXClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e) {
    ...
    $.post('https://wickx-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e) {
    ...
    $.post('https://wickx-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
WKXClothing.Close = function() {
    $.post('https://wickx-clothing/close');
...
WKXClothing.SetMaxValues = function(maxValues) {
...
WKXClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e) {
    ...
    $.post('https://wickx-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e) {
    ...
    $.post('https://wickx-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
