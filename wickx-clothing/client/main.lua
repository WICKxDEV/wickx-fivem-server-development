...
local function SaveSkin()
    ...
    TriggerServerEvent("wickx-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = WKXCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("wickx-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("wickx-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if WKXCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    WKXCore.Functions.TriggerCallback('wickx-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('wickx-clothing:client:openMenu', function()
...
RegisterNetEvent('wickx-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('wickx-clothes:client:CreateFirstCharacter', function()
    WKXCore.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("wickx-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('wickx-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('wickx-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('wickx-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if WKXCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        WKXCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("wickx-clothing:client:adjustfacewear", function(type)
    if WKXCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('WKXCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("wickx-clothes:loadPlayerSkin")
    PlayerData = WKXCore.Functions.GetPlayerData()
    loadStores()
--    WKXCore.Shared.Jobs = exports['wickx-jobs']:AddJobs()
end)
RegisterNetEvent('WKXCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('WKXCore:Client:OnGangUpdate', function(GangInfo)
    PlayerData.gang = GangInfo
end)
RegisterNetEvent('wickx-clothing:client:openOutfitMenu', function()
    WKXCore.Functions.TriggerCallback('wickx-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('wickx-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    WKXCore.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('wickx-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('wickx-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('wickx-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    WKXCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['wickx-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not WKXCore.Shared.WKXJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                        local gradeLevel = PlayerData.job.grade.level
                        getOutfits(gradeLevel, Config.Outfits[v.requiredJob])
                    end
                else break end --this break is important if WKXJobsStatus is true then the reste of the code in this loop does not need to run.

                exports['wickx-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['wickx-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['wickx-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['wickx-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['wickx-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not WKXCore.Shared.WKXJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(WKXCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not WKXCore.Shared.WKXJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not WKXCore.Shared.WKXJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not WKXCore.Shared.WKXJobsStatus and PlayerData.job.grade.level)
