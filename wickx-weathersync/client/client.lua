...
RegisterNetEvent('WKXCore:Client:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('wickx-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('wickx-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('wickx-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('wickx-weathersync:client:DisableSync', function()
    disable = true
    SetRainLevel(0.0)
...
RegisterNetEvent('wickx-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('wickx-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)
