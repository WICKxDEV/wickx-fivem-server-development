...
AddEventHandler('WKXCore:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent('wickx-vehicleshop:server:addPlayer', citizenid)
    TriggerServerEvent('wickx-vehicleshop:server:checkFinance')
...
RegisterNetEvent('WKXCore:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('wickx-vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'wickx-vehicleshop:client:showVehOptions'
        }
    }
}
...
local financeMenu = {
    {
        ...
        params = {
            event = 'wickx-vehicleshop:client:getVehicles'
        }
    }
}
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'wickx-vehicleshop:client:TestDriveReturn'
        }
    }
}
...
local function getVehName()
    return WKXCore.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['name']
end

local function getVehPrice()
    return comma_value(WKXCore.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['price'])
end

local function getVehBrand()
    return WKXCore.Shared.Vehicles[Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle]['brand']
end
...
local function createTestDriveReturn()
    ...
    testDriveZone:onPlayerInOut(function(isPointInside)
        if isPointInside and IsPedInAnyVehicle(PlayerPedId()) then
            ...
            exports['wickx-menu']:openMenu(returnTestDrive)
        else
        ...
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if secondsLeft >= tonumber(1000 * testDriveTime) - 20 or GetPedInVehicleSeat(NetToVeh(testDriveVeh), -1) ~= PlayerPedId() then
                    TriggerServerEvent('wickx-vehicleshop:server:deleteVehicle', testDriveVeh)
                    testDriveVeh = 0
                    inTestDrive = false
                    SetEntityCoords(PlayerPedId(), prevCoords)
                    WKXCore.Functions.Notify(Lang:t('general.testdrive_complete'))
                end
...
local function createVehZones(shopName, entity)
    if not Config.UsingTarget then
        ...
        combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if PlayerData and PlayerData.job and (PlayerData.job.name == Config.Shops[insideShop]['Job'] or Config.Shops[insideShop]['Job'] == 'none') then
                    exports['wickx-menu']:showHeader(vehHeaderMenu)
                end
            else
                exports['wickx-menu']:closeMenu()
            end
        ...
        exports['wickx-target']:AddTargetEntity(entity, {
            options = {
                {
                    type = 'client',
                    event = 'wickx-vehicleshop:client:showVehOptions',
                ...
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'wickx-vehicleshop:client:TestDrive',
                            }
                        },
                        {
                            ...
                            params = {
                                isServer = true,
                                event = 'wickx-vehicleshop:server:buyShowroomVehicle',
                                args = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'wickx-vehicleshop:client:openFinance',
                                args = {
                                    price = getVehPrice(),
                                    buyVehicle = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle
                                }
                        ...
                        },
                        {
                            ...
                            params = {
                                event = Config.FilterByMake and 'wickx-vehicleshop:client:vehMakes' or 'wickx-vehicleshop:client:vehCategories',
                            }
                        },
                    }
                    Wait(1000)
                ...
                while insideShop and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'wickx-vehicleshop:client:openIdMenu',
                                args = {
                        ...
                        },
                        {
                            ...
                            icon = 'fa-solid fa-coins',
                            params = {
                                event = 'wickx-vehicleshop:client:openCustomFinance',
                                args = {
                                    price = getVehPrice(),
                                    vehicle = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle
                                }
...
local function createFinanceZone(coords, name)
    ...
    financeZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            exports['wickx-menu']:showHeader(financeMenu)
        else
            exports['wickx-menu']:closeMenu()
        end
    end)
...
-- Events
RegisterNetEvent('wickx-vehicleshop:client:homeMenu', function()
    exports['wickx-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('wickx-vehicleshop:client:showVehOptions', function()
    exports['wickx-menu']:openMenu(vehicleMenu, true, true)
end)

RegisterNetEvent('wickx-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        inTestDrive = true
        local prevCoords = GetEntityCoords(PlayerPedId())
        tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
        WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
            local veh = NetToVeh(netId)
            exports['LegacyFuel']:SetFuel(veh, 100)
            SetVehicleNumberPlateText(veh, 'TESTDRIVE')
            SetEntityHeading(veh, Config.Shops[tempShop]['TestDriveSpawn'].w)
            TriggerEvent('vehiclekeys:client:SetOwner', WKXCore.Functions.GetPlate(veh))
            testDriveVeh = netId
            WKXCore.Functions.Notify(Lang:t('general.testdrive_timenoti', { testdrivetime = Config.Shops[tempShop]['TestDriveTimeLimit'] }))
        ...
        WKXCore.Functions.Notify(Lang:t('error.testdrive_alreadyin'), 'error')
    end
end)

RegisterNetEvent('wickx-vehicleshop:client:customTestDrive', function(data)
...
RegisterNetEvent('wickx-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == entity then
        ...
        WKXCore.Functions.Notify(Lang:t('error.testdrive_return'), 'error')
    end
end)

RegisterNetEvent('wickx-vehicleshop:client:vehCategories', function(data)
    local catmenu = {}
    ...
    local categoryMenu = {
        {
            ...
            params = {
                event = Config.FilterByMake and 'wickx-vehicleshop:client:vehMakes' or 'wickx-vehicleshop:client:homeMenu'
            }
        }
    }
    for k, v in pairs(WKXCore.Shared.Vehicles) do
        if type(WKXCore.Shared.Vehicles[k]['shop']) == 'table' then
            for _, shop in pairs(WKXCore.Shared.Vehicles[k]['shop']) do
                if shop == insideShop and (not Config.FilterByMake or WKXCore.Shared.Vehicles[k]['brand'] == data.make) then
        ...
        elseif WKXCore.Shared.Vehicles[k]['shop'] == insideShop and (not Config.FilterByMake or WKXCore.Shared.Vehicles[k]['brand'] == data.make) then
            catmenu[v.category] = v.category
            if firstvalue == nil then
                firstvalue = v.category
            end
    ...
    if Config.HideCategorySelectForOne and tablelength(catmenu) == 1 then
        TriggerEvent('wickx-vehicleshop:client:openVehCats', { catName = firstvalue, make = Config.FilterByMake and data.make, onecat = true })
        return
    end
    for k, v in pairs(catmenu) do
        categoryMenu[#categoryMenu + 1] = {
            ...
            params = {
                event = 'wickx-vehicleshop:client:openVehCats',
                args = {
                    catName = k,
                }
            }
    ...
    exports['wickx-menu']:openMenu(categoryMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('wickx-vehicleshop:client:openVehCats', function(data)
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'wickx-vehicleshop:client:vehCategories',
                args = {
                    make = data.make
                }
            }
    ...
    if data.onecat == true then
        vehMenu[1].params = {
            event = 'wickx-vehicleshop:client:vehMakes'
        }
    end
    for k, v in pairs(WKXCore.Shared.Vehicles) do
        if WKXCore.Shared.Vehicles[k]['category'] == data.catName then
            if type(WKXCore.Shared.Vehicles[k]['shop']) == 'table' then
                for _, shop in pairs(WKXCore.Shared.Vehicles[k]['shop']) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                isServer = true,
                                event = 'wickx-vehicleshop:server:swapVehicle',
                                args = {
                                    toVehicle = v.model,
            ...
            elseif WKXCore.Shared.Vehicles[k]['shop'] == insideShop then
                vehMenu[#vehMenu + 1] = {
                    header = v.name,
                    txt = Lang:t('menus.veh_price') .. v.price,
                    icon = 'fa-solid fa-car-side',
    ...
    exports['wickx-menu']:openMenu(vehMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('wickx-vehicleshop:client:vehMakes', function()
    local makmenu = {}
    local makeMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'wickx-vehicleshop:client:homeMenu'
            }
        }
    }
    for k, v in pairs(WKXCore.Shared.Vehicles) do
    ...
    exports['wickx-menu']:openMenu(makeMenu, Config.SortAlphabetically, true)
end)

RegisterNetEvent('wickx-vehicleshop:client:openFinance', function(data)
    local dialog = exports['wickx-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('wickx-vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
    end
end)

RegisterNetEvent('wickx-vehicleshop:client:openCustomFinance', function(data)
    local dialog = exports['wickx-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('wickx-vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.playerid)
    end
end)

RegisterNetEvent('wickx-vehicleshop:client:swapVehicle', function(data)
    local shopName = data.ClosestShop
    if Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        local closestVehicle, closestDistance = WKXCore.Functions.GetClosestVehicle(vector3(Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.x, Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.y, Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].coords.z))
...
RegisterNetEvent('wickx-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
    WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
        local veh = NetToVeh(netId)
        exports['LegacyFuel']:SetFuel(veh, 100)
        ...
        TriggerEvent('vehiclekeys:client:SetOwner', WKXCore.Functions.GetPlate(veh))
        TriggerServerEvent('wickx-mechanicjob:server:SaveVehicleProps', WKXCore.Functions.GetVehicleProperties(veh))
    end, vehicle, Config.Shops[tempShop]['VehicleSpawn'], true)
end)

RegisterNetEvent('wickx-vehicleshop:client:getVehicles', function()
    WKXCore.Functions.TriggerCallback('wickx-vehicleshop:server:getVehicles', function(vehicles)
        local ownedVehicles = {}
        for _, v in pairs(vehicles) do
            local vehData = WKXCore.Shared.Vehicles[v.vehicle]
            if v.balance ~= 0 and vehData.shop == insideShop then
                ...
                ownedVehicles[#ownedVehicles + 1] = {
                    ...
                    icon = 'fa-solid fa-car-side',
                    params = {
                        event = 'wickx-vehicleshop:client:getVehicleFinance',
                        args = {
                            vehiclePlate = plate,
                            balance = v.balance,
        ...
        if #ownedVehicles > 0 then
            exports['wickx-menu']:openMenu(ownedVehicles)
        else
            WKXCore.Functions.Notify(Lang:t('error.nofinanced'), 'error', 7500)
        end
    end)
end)

RegisterNetEvent('wickx-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        {
            header = Lang:t('menus.goback_header'),
            params = {
        ...
        },
        {
            ...
            icon = 'fa-solid fa-hand-holding-dollar',
            params = {
                event = 'wickx-vehicleshop:client:financePayment',
                args = {
                    vehData = data,
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = 'wickx-vehicleshop:server:financePaymentFull',
                args = {
    ...
    exports['wickx-menu']:openMenu(vehFinance)
end)

RegisterNetEvent('wickx-vehicleshop:client:financePayment', function(data)
    local dialog = exports['wickx-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('wickx-vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
    end
end)

RegisterNetEvent('wickx-vehicleshop:client:openIdMenu', function(data)
    local dialog = exports['wickx-input']:ShowInput({
        header = WKXCore.Shared.Vehicles[data.vehicle]['name'],
        submitText = Lang:t('menus.submit_text'),
        inputs = {
            {
                text = Lang:t('menus.submit_ID'),
    ...
    if dialog then
        ...
        if data.type == 'testDrive' then
            TriggerServerEvent('wickx-vehicleshop:server:customTestDrive', data.vehicle, dialog.playerid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('wickx-vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.playerid)
        end
    end
end)
