local function GetArmor(vehicle)
    ...
    for i = -1, GetNumVehicleMods(vehicle, 16) - 1 do
        ...
        local armorItem = {
            ...
            disabled = disabled,
            params = {
                event = 'wickx-mechanicjob:client:install',
                args = {
                    upgradeType = 'armor',
                    modType = 16,
                    upgradeIndex = i,
    ...
    exports['wickx-menu']:openMenu(armorMenu, true)
end
...
local function GetBrakes(vehicle)
    ...
    exports['wickx-menu']:openMenu(brakesMenu, true)
end
...
local function GetEngine(vehicle)
    ...
    exports['wickx-menu']:openMenu(engineMenu, true)
end
...
local function GetSuspension(vehicle)
    ...
    exports['wickx-menu']:openMenu(suspensionMenu, true)
end
...
local function GetTransmission(vehicle)
    ...
    exports['wickx-menu']:openMenu(transmissionMenu, true)
end
...
local function GetTurbo(vehicle)
    ...
    exports['wickx-menu']:openMenu(turboMenu, true)
end

-- Events

RegisterNetEvent('wickx-mechanicjob:client:install', function(data)
    ...
    WKXCore.Functions.Progressbar('mechanic_install', string.format(Lang:t('progress.installing'), partName), 5000, false, true, {
        ...
        TriggerServerEvent('wickx-mechanicjob:server:removeItem', 'veh_' .. partName)
        WKXCore.Functions.Notify(string.format(Lang:t('success.installed'), partName), 'success')
    end, function()
        if shouldToggleHood then ToggleHood(data.vehicle) end
    end)
end)

RegisterNetEvent('wickx-mechanicjob:client:installPart', function(item)
