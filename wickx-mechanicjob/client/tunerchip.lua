...
RegisterNUICallback('reset', function(_, cb)
    ResetVeh()
    TriggerEvent('wickx-mechanicjob:client:openChip')
    cb('ok')
end)
...
RegisterNetEvent('wickx-mechanicjob:client:checkTune', function()
    local vehicle, distance = WKXCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    local plate = WKXCore.Functions.GetPlate(vehicle)
    if not plate then return end
    WKXCore.Functions.TriggerCallback('wickx-mechanicjob:server:checkTune', function(status)
        if status then
            WKXCore.Functions.Notify(Lang:t('success.tuned'), 'success')
        else
            WKXCore.Functions.Notify(Lang:t('warning.not_tuned'), 'error')
        end
    end, plate)
end)

RegisterNetEvent('wickx-mechanicjob:client:openChip', function()
    ...
    if inVehicle then
        ...
        local plate = WKXCore.Functions.GetPlate(vehicle)
        WKXCore.Functions.Progressbar('connect_laptop', Lang:t('progress.tuner_chip'), 2000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function()
            TriggerServerEvent('wickx-mechanicjob:server:tuneStatus', plate)
