        ...
        while true do
            ...
            if isDriver then
                if IsControlJustPressed(0, 155) then
                    ...
                    TriggerServerEvent('wickx-mechanicjob:server:syncNitrousFlames', netId, true)
                    nitrousActive = true
                end
                ...
                if IsControlJustReleased(0, 155) or nitrousVehicles[plate].level <= 0 then
                    ...
                    TriggerServerEvent('wickx-mechanicjob:server:syncNitrousFlames', netId, false)
                    if nitrousVehicles[plate].level <= 0 then
                        nitrousVehicles[plate].hasnitro = false
                        TriggerServerEvent('wickx-mechanicjob:server:syncNitrous', plate, false)
                        ...
                        TriggerServerEvent('wickx-mechanicjob:server:syncNitrous', plate, true, nitrousVehicles[plate].level)
                    end
                end
            else
                plate = nil
...
AddEventHandler('gameEventTriggered', function(event)
    if event == 'CEventNetworkPlayerEnteredVehicle' then
        ...
        WKXCore.Functions.TriggerCallback('wickx-mechanicjob:server:getnitrousVehicles', function(vehs)
...
RegisterNetEvent('wickx-mechanicjob:client:syncNitrousFlames', function(net, toggle)
...
RegisterNetEvent('wickx-mechanicjob:client:installNitrous', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then return end
    local closestVehicle, distance = WKXCore.Functions.GetClosestVehicle()
    ...
    local vehiclePlate = WKXCore.Functions.GetPlate(closestVehicle)
    if not vehiclePlate then return end
    if not IsNearBone(closestVehicle, 'engine') then return end
    ToggleHood(closestVehicle)
    WKXCore.Functions.Progressbar('use_nos', Lang:t('progress.nitrous'), 5000, false, true, {
        ...
        ToggleHood(closestVehicle)
        TriggerServerEvent('wickx-mechanicjob:server:removeItem', 'nitrous')
        TriggerServerEvent('wickx-mechanicjob:server:syncNitrous', vehiclePlate, true, 100)
