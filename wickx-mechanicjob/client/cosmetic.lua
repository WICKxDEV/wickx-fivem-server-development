...
local function PaintList(category)
    ...
    if dialog.primarypaint and dialog.secondarypaint and dialog.pearlescentpaint and dialog.wheelpaint then
        ...
        TriggerServerEvent('wickx-mechanicjob:server:sprayVehicle', netId, dialog.primarypaint, dialog.secondarypaint, dialog.pearlescentpaint, dialog.wheelpaint, colors)
    end
end
...
local function CustomColor()
    ...
    if (dialog.hex or dialog.colorpicker) and dialog.section then
        ...
        local netId = NetworkGetNetworkIdFromEntity(vehicle)
        if isPainting then return end
        TriggerServerEvent('wickx-mechanicjob:server:sprayVehicleCustom', netId, dialog.section, dialog.paintType, HexToRGB(color))
    end
end
...
function PaintCategories()
    ...
    exports['wickx-menu']:openMenu(Paints)
end
...
local function OpenInteriors(vehicle)
    ...
    exports['wickx-menu']:openMenu(mods)
end
...
local function WindowTint(vehicle)
    ...
    exports['wickx-menu']:openMenu(tints)
end
...
local function PlateIndex(vehicle)
    ...
    exports['wickx-menu']:openMenu(plates)
end

-- Events

RegisterNetEvent('wickx-mechanicjob:client:vehicleSetColors', function(netId, section, colorIndex)
    ...
    local props = WKXCore.Functions.GetVehicleProperties(vehicle)
    TriggerServerEvent('wickx-mechanicjob:server:SaveVehicleProps', props)
end)

RegisterNetEvent('wickx-mechanicjob:client:startParticles', function(netId, color)
    isPainting = true
    local vehicle = NetworkGetEntityFromNetworkId(netId)
...
RegisterNetEvent('wickx-mechanicjob:client:stopParticles', function()
    isPainting = false
...
RegisterNetEvent('wickx-mechanicjob:client:installCosmetic', function(item)
    local vehicle, distance = WKXCore.Functions.GetClosestVehicle()
    if vehicle == 0 or distance > 5.0 then return end
    ...
    if item == 'veh_interior' then
        OpenInteriors(vehicle)
        TriggerServerEvent('wickx-mechanicjob:server:removeItem', item)
