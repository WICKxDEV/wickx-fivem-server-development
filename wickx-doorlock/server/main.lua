...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and WKXCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
WKXCore.Functions.CreateCallback('wickx-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

WKXCore.Functions.CreateCallback('wickx-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('wickx-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('wickx-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('wickx-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('wickx-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not WKXCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = WKXCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('wickx-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('wickx-doorlock:server:removeLockpick', function(type)
	local Player = WKXCore.Functions.GetPlayer(source)
...
WKXCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('wickx-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

WKXCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('wickx-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
