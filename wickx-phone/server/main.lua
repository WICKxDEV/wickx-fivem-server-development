...
local function GetOnlineStatus(number)
    local Target = WKXCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
function WKXPhone.AddMentionedTweet(citizenid, TweetData)
...
function WKXPhone.SetPhoneAlerts(citizenid, app, alerts)
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('wickx-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('wickx-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
WKXCore.Functions.CreateCallback("wickx-phone:server:GetInvoices", function(source, cb)
    ...
    if Player then
        ...
        for _, v in pairs(invoices) do
            local Ply = WKXCore.Functions.GetPlayerByCitizenId(v.sender)
            if Ply ~= nil then
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = WKXCore.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
WKXCore.Functions.CreateCallback('wickx-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Ply = WKXCore.Functions.GetPlayer(source)
    local SenderPly = WKXCore.Functions.GetPlayerByCitizenId(sendercitizenid)
        ...
        if exists[1] and exists[1]["count"] == 1 then
            ...
            if Ply.Functions.RemoveMoney('bank', amount, 'paid-invoice') then
                ...
                if invoiceMailData then
                    exports['wickx-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
                end
                TriggerEvent("wickx-phone:server:paidInvoice", source, invoiceId)
                exports['wickx-banking']:AddMoney(society, amount, 'Phone invoice')
                cb(true)
                return
            end
        end
...
WKXCore.Functions.CreateCallback('wickx-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Ply = WKXCore.Functions.GetPlayer(source)
    if Ply then
        ...
        if exists[1] and exists[1]["count"] == 1 then
            TriggerEvent("wickx-phone:server:declinedInvoice", source, invoiceId)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetContactPictures', function(_, cb, Chats)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetContactPicture', function(_, cb, Chat)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
WKXCore.Functions.CreateCallback('wickx-phone:server:FetchResult', function(_, cb, search)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = WKXCore.Shared.Vehicles[result[k].vehicle]
...
WKXCore.Functions.CreateCallback('wickx-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('WKXCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

WKXCore.Functions.CreateCallback('wickx-phone:server:HasPhone', function(source, cb)
    local Player = WKXCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
WKXCore.Functions.CreateCallback('wickx-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
        if result[1] ~= nil then
            local Reciever = WKXCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
WKXCore.Functions.CreateCallback('wickx-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        cb(WebHook)
    else
...
WKXCore.Functions.CreateCallback('wickx-phone:server:UploadToFivemerr', function(source, cb)
    local src = source
...
RegisterNetEvent('wickx-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('wickx-phone:client:UpdateAdverts', -1, Adverts, '@' .. Player.PlayerData.charinfo.firstname .. '' .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('wickx-phone:server:DeleteAdvert', function()
    local Player = WKXCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('wickx-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('wickx-phone:server:SetCallState', function(bool)
    local src = source
    local Ply = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
...
RegisterNetEvent('wickx-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = WKXCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('wickx-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('wickx-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(WKXCore.Functions.GetPlayers()) do
        local Player = WKXCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                WKXPhone.SetPhoneAlerts(Player.PlayerData.citizenid, 'twitter')
                WKXPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                TriggerClientEvent('wickx-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]['twitter'])
            else
                ...
                if result[1] ~= nil then
                    ...
                    WKXPhone.SetPhoneAlerts(MentionedTarget, 'twitter')
                    WKXPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                end
            end
        end
    end
...
RegisterNetEvent('wickx-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = WKXCore.Functions.GetPlayer(src)
    local Target = WKXCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('wickx-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('wickx-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(WKXCore.Functions.GetPlayers()) do
        local target = WKXCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. WKXCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. WKXCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('wickx-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. WKXCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. WKXCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('wickx-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('wickx-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('wickx-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('wickx-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = WKXCore.Functions.GetPlayer(src).citizenid
    WKXPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('wickx-phone:server:DeleteTweet', function(tweetId)
    local Player = WKXCore.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('wickx-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('wickx-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('wickx-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('wickx-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = WKXCore.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('wickx-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('WKXCore:Notify', src, "This account number doesn't exist!", 'error')
    end
end)

RegisterNetEvent('wickx-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('wickx-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = WKXCore.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if Player[1] ~= nil then
        local TargetData = WKXCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('wickx-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('wickx-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', { SenderData.PlayerData.citizenid, ChatNumber })
            if Chat[1] ~= nil then
...
RegisterNetEvent('wickx-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = WKXCore.Functions.GetPlayer(src)
    local Hour = os.date('%H')
    local Minute = os.date('%M')
    local label = Hour .. ':' .. Minute
    TriggerClientEvent('wickx-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = WKXCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('wickx-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('wickx-phone:server:CancelCall', function(ContactData)
    local Ply = WKXCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('wickx-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('wickx-phone:server:AnswerCall', function(CallData)
    local Ply = WKXCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('wickx-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('wickx-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('wickx-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('wickx-phone:server:AddTransaction', function(data)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('wickx-phone:RefreshPhone', src)
end)

RegisterNetEvent('wickx-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)', { Player.PlayerData.citizenid, image })
end)

RegisterNetEvent('wickx-phone:server:getImageFromGallery', function()
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC', { Player.PlayerData.citizenid })
    TriggerClientEvent('wickx-phone:refreshImages', src, images)
end)

RegisterNetEvent('wickx-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
...
RegisterNetEvent('wickx-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('WKXCore:Notify', src, 'You cannot ping yourself', 'error')
    end
end)

-- Command

WKXCore.Commands.Add('setmetadata', 'Set Player Metadata (God Only)', {}, false, function(source, args)
    local Player = WKXCore.Functions.GetPlayer(source)
    if args[1] then
...
WKXCore.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' } }, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('WKXCore:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('WKXCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('WKXCore:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('WKXCore:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('WKXCore:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('WKXCore:Notify', source, 'No Access', 'error')
    end
end)
