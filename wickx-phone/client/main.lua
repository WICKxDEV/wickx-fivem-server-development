...
local function findVehFromPlateAndLocate(plate)
    local gameVehicles = WKXCore.Functions.GetVehicles()
    for i = 1, #gameVehicles do
        local vehicle = gameVehicles[i]
        if DoesEntityExist(vehicle) then
            if WKXCore.Functions.GetPlate(vehicle) == plate then
...
local function LoadPhone()
    Wait(100)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetPhoneData', function(pData)
        PlayerJob = WKXCore.Functions.GetPlayerData().job
        PhoneData.PlayerData = WKXCore.Functions.GetPlayerData()
...
local function OpenPhone()
    WKXCore.Functions.TriggerCallback('wickx-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = WKXCore.Functions.GetPlayerData()
            ...
            WKXCore.Functions.TriggerCallback('wickx-garages:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            WKXCore.Functions.Notify("You don't have a phone", 'error')
        end
    end)
end
...
local function CancelCall()
    TriggerServerEvent('wickx-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('wickx-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('wickx-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('wickx-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('wickx-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            WKXCore.Functions.Notify('Action not available at the moment..', 'error')
        end
    end
end)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'phone', 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('wickx-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('wickx-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('wickx-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('wickx-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('GetInvoices', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetInvoices', function(resp)
        cb(resp)
    end)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('wickx-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback('DeleteAdvert', function(_, cb)
    TriggerServerEvent('wickx-phone:server:DeleteAdvert')
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'whatsapp', newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    WKXCore.Functions.TriggerCallback('wickx-phone:server:PayInvoice', function(resp)
        cb(resp)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('wickx-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    WKXCore.Functions.TriggerCallback('wickx-phone:server:DeclineInvoice', function(resp)
        cb(resp)
    end, society, amount, invoiceId)
    TriggerServerEvent('wickx-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('wickx-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                TriggerServerEvent('wickx-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                if (Firstname ~= nil and Firstname ~= '') and (Lastname ~= nil and Lastname ~= '') then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('wickx-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('wickx-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet', function(data, cb)
    TriggerServerEvent('wickx-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('wickx-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('wickx-phone:server:RemoveInstallation', data.app)
    cb('ok')
end)

RegisterNUICallback('GetTruckerData', function(_, cb)
    local TruckerMeta = WKXCore.Functions.GetPlayerData().metadata['jobrep']['trucker']
    local TierData = exports['wickx-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('wickx-phone:server:RemoveImageFromGallery', image)
    Wait(400)
    TriggerServerEvent('wickx-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        WKXCore.Functions.Notify('Your vehicle has been marked', 'success')
    else
        WKXCore.Functions.Notify('This vehicle cannot be located', 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('wickx-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data, cb)
    TriggerServerEvent('wickx-lapraces:server:JoinRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('LeaveRace', function(data, cb)
    TriggerServerEvent('wickx-lapraces:server:LeaveRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('StartRace', function(data, cb)
    TriggerServerEvent('wickx-lapraces:server:StartRace', data.RaceData.RaceId)
    cb('ok')
end)

RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    local coords = data.alert.coords
    WKXCore.Functions.Notify('GPS Location set: ' .. data.alert.title)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, _ in pairs(result) do
                WKXCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    local vehicle = WKXCore.Functions.GetClosestVehicle()
    local plate = WKXCore.Functions.GetPlate(vehicle)
    local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
    WKXCore.Functions.TriggerCallback('wickx-phone:server:ScanPlate', function(result)
        WKXCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            if WKXCore.Shared.Vehicles[vehname] ~= nil then
                result.label = WKXCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('GetRaces', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('wickx-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
    cb('ok')
end)

RegisterNUICallback('HasCreatedRace', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(_, cb)
    local InRace = exports['wickx-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['wickx-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('wickx-lapraces:server:CreateLapRace', data.TrackName)
    cb('ok')
end)

RegisterNUICallback('GetRacingLeaderboards', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('wickx-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            WKXCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == 'editor' then
        cb(exports['wickx-lapraces']:IsInEditor())
    else
        cb(exports['wickx-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    WKXCore.Functions.Notify('GPS has been set to ' .. data.HouseData.HouseData.adress .. '!', 'success')
    cb('ok')
end)

RegisterNUICallback('RemoveKeyholder', function(data, cb)
    TriggerServerEvent('wickx-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    WKXCore.Functions.TriggerCallback('wickx-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    WKXCore.Functions.Notify('GPS has been set!', 'success')
    cb('ok')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_, cb)
    ...
    TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'twitter', 0)
    SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('wickx-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        WKXCore.Functions.TriggerCallback('wickx-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('wickx-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('wickx-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    WKXCore.Functions.TriggerCallback('wickx-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = 'UpdateChat',
            chatData = Chat,
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            if Config.Fivemerr == true then
                ...
                return WKXCore.Functions.TriggerCallback('wickx-phone:server:UploadToFivemerr', function(fivemerrData)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('wickx-phone:server:addImageToGallery', imageData.url)
                    Wait(400)
                    TriggerServerEvent('wickx-phone:server:getImageFromGallery')
            ...
            WKXCore.Functions.TriggerCallback('wickx-phone:server:GetWebhook', function(hook)
                if not hook then
                    WKXCore.Functions.Notify('Camera not setup', 'error')
                    return
                end
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), 'files[]', function(data)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('wickx-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
                    TriggerServerEvent('wickx-phone:server:getImageFromGallery')
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        WKXCore.Functions.Notify('You need to input a Player ID', 'error')
    else
        TriggerServerEvent('wickx-phone:server:sendPing', args[1])
    end
end, false)
...
RegisterNetEvent('wickx-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'WKXBank', text = '&#36;' .. amount .. ' has been added to your account!', icon = 'fas fa-university', color = '#8c7ae6', }, })
    SendNUIMessage({ action = 'UpdateBank', NewBalance = PhoneData.PlayerData.money.bank })
end)

-- RegisterNetEvent('wickx-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('wickx-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('wickx-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Racing',
...
RegisterNetEvent('wickx-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'phone')
...
RegisterNetEvent('wickx-phone-new:client:BankNotify', function(text)
    SendNUIMessage({
        action = 'PhoneNotification',
        NotifyData = {
            title = 'Bank',
...
RegisterNetEvent('wickx-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Mail',
    ...
    TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'mail')
end)

RegisterNetEvent('wickx-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = 'UpdateMails',
        Mails = NewMails
    })
...
RegisterNetEvent('wickx-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('wickx-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
...
RegisterNetEvent('wickx-phone:client:BillingEmail', function(data, paid, name)
    if paid then
...
RegisterNetEvent('wickx-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == 'ongoing' then
        SendNUIMessage({
...
RegisterNetEvent('wickx-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('wickx-phone:server:AddRecentCall', 'missed', CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('wickx-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'whatsapp')
        end
    else
...
RegisterNetEvent('wickx-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNetEvent('wickx-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('wickx-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('wickx-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('wickx-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications['phone'].Alerts = Config.PhoneApplications['phone'].Alerts + 1
    TriggerServerEvent('wickx-phone:server:SetPhoneAlerts', 'phone', Config.PhoneApplications['phone'].Alerts)
end)

RegisterNetEvent('wickx-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('wickx-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = 'ongoing'
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('wickx-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = WKXCore.Functions.GetPlayerData().job
...
RegisterNetEvent('wickx-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('wickx-phone:server:GiveContactDetails', PlayerId)
    else
        WKXCore.Functions.Notify('No one nearby!', 'error')
    end
end)

RegisterNetEvent('wickx-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = 'UpdateRacingApp',
    })
end)

RegisterNetEvent('wickx-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('wickx-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent('wickx-phone:client:CustomNotification', function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = title,
