...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    WKX.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://wickx-phone/ClearAlerts', JSON.stringify({
...
WKX.Phone.Functions.GetLastMessage = function(messages) {
...
WKX.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = WKX.Phone.Functions.GetLastMessage(chat.messages);
...
WKX.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://wickx-phone/SendMessage', JSON.stringify({
        ...
        WKX.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
WKX.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        OpenedChatData.number = cData.number;

        if (OpenedChatPicture == null) {
            $.post('https://wickx-phone/GetProfilePicture', JSON.stringify({
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var Sender = "me";
                if (message.sender !== WKX.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
                var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://wickx-phone/SharedLocation', JSON.stringify({
