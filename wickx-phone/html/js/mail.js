...
$(document).on('click', '.mail', function(e){
    ...
    WKX.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://wickx-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://wickx-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
WKX.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(WKX.Phone.Data.PlayerData.charinfo.firstname+"."+WKX.Phone.Data.PlayerData.charinfo.lastname+"@wickxcore.com");
...
WKX.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://wickx-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://wickx-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
        }else {
        ...
        WKX.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

WKX.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+WKX.Phone.Data.PlayerData.charinfo.firstname+""+WKX.Phone.Data.PlayerData.charinfo.lastname+" | "+WKX.Phone.Data.PlayerData.charinfo.phone);
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (advert.number === WKX.Phone.Data.PlayerData.charinfo.phone){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://wickx-phone/DeleteAdvert', function(){
        setTimeout(function(){
            WKX.Phone.Notifications.Add("fas fa-ad", "Advertisement", "The ad was deleted", "#ff8f1a", 2000);
        },400)
    });
})
