...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://wickx-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    WKX.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
WKX.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = WKX.Phone.Functions.FormatTwitterMessage(clean);
            ...
            // if (Tweet.citizenid === WKX.Phone.Data.PlayerData.citizenid){
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://wickx-phone/DeleteTweet', JSON.stringify({id: source}))
})
...
WKX.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
WKX.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://wickx-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: WKX.Phone.Data.MetaData.profilepicture,
            url: imageURL
        }), function(Tweets){
            WKX.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        WKX.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        WKX.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    };
    $('#tweet-new-url').val("");
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://wickx-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            WKX.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    WKX.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

WKX.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
WKX.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    WKX.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
