                ...
                if dist < 25 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('wickx-taxi:server:NpcPay', meterData.currentFare, NpcData.CrashCount == 0)
                            ...
                            WKXCore.Functions.Notify(Lang:t('info.person_was_dropped_off'), 'success')
                            if NpcData.DeliveryBlip ~= nil then
                                RemoveBlip(NpcData.DeliveryBlip)
                            end
            ...
            if NpcData.NpcTaken then
                ...
                if vehicle and vehicle ~= 0 then
                    ...
                    if currentHealth < Config.Advanced.MinCabHealth then
                        TriggerEvent('wickx-taxi:client:CancelTaxiNpc')
                        return WKXCore.Functions.Notify(Lang:t('error.broken_taxi'), 'error')
                    end
                    ...
                    if lastVehicleHealth and currentHealth < lastVehicleHealth then
                        ...
                        if NpcData.CrashCount >= Config.Advanced.MaxCrashesAllowed then
                            TriggerEvent('wickx-taxi:client:CancelTaxiNpc')
                            return WKXCore.Functions.Notify(Lang:t('error.ride_canceled'), 'error')
                        end

                        local count = Config.Advanced.MaxCrashesAllowed - NpcData.CrashCount
                        WKXCore.Functions.Notify(string.format(Lang:t('error.crash_warning'), count, count == 1 and Lang:t('error.time') or Lang:t('error.times')), 'error')
                    end
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = 'wickx-taxi:client:TakeVehicle',
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent('wickx-taxi:client:TakeVehicle', function(data)
    ...
    if SpawnPoint then
        ...
        if CanSpawn then
            ...
            WKXCore.Functions.Notify(Lang:t('info.no_spawn_point'), 'error')
        end
    else
        WKXCore.Functions.Notify(Lang:t('info.no_spawn_point'), 'error')
        return
...
-- Events
RegisterNetEvent('wickx-taxi:client:DoTaxiNpc', function()
    ...
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            WKXCore.Functions.Notify(Lang:t('info.npc_on_gps'), 'success')
                        ...
                        if dist < 25 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    resetMeter()
                                    WKXCore.Functions.Notify(Lang:t('info.go_to_location'))
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
            ...
            WKXCore.Functions.Notify(Lang:t('error.already_mission'))
        end
    else
        WKXCore.Functions.Notify(Lang:t('error.not_in_taxi'))
    end
end)

RegisterNetEvent('wickx-taxi:client:CancelTaxiNpc', function()
...
RegisterNetEvent('wickx-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            WKXCore.Functions.Notify(Lang:t('error.missing_meter'), 'error')
        end
    else
...
RegisterNetEvent('wickx-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = 'toggleMeter'
        })
    else
        WKXCore.Functions.Notify(Lang:t('error.not_active_meter'), 'error')
    end
end)
...
RegisterNetEvent('wickx-taxijob:client:requestcab', function()
    TaxiGarage()
end)
    ...
    while true do
        if not Config.UseTarget then
            ...
            if LocalPlayer.state.isLoggedIn then
                local Player = WKXCore.Functions.GetPlayerData()
                            ...
                            if whitelistedVehicle() then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if PlayerJob.onduty then
                                        ...
                                        WKXCore.Functions.Notify('You need to be on duty')
                                    end
                                end
                            end
                        end
        ...
        exports['wickx-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = 'client',
                        event = 'wickx-taxijob:client:requestcab',
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['wickx-core']:DrawText(Lang:t('info.call_npc'), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    deliveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['wickx-core']:DrawText(Lang:t('info.drop_off_npc'), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['wickx-core']:DrawText(Lang:t('info.vehicle_parking'), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('wickx-taxi:client:toggleMeter')
                        meterActive = false
                    end
                    TaskLeaveVehicle(PlayerPedId(), vehicle, 0)
                    Wait(2000) -- 2 second delay just to ensure the player is out of the vehicle
                    DeleteVehicle(vehicle)
                    WKXCore.Functions.Notify(Lang:t('info.taxi_returned'), 'success')
                end
            end
        end
        Wait(1)
