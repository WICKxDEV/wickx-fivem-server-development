...
local function getJobs()
    WKXCore.Functions.TriggerCallback('wickx-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('wickx-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['wickx-core']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['wickx-core']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('wickx-cityhall:Client:AddCityJob', function()
    getJobs()
end)

RegisterNetEvent('wickx-cityhall:client:getIds', function()
    TriggerServerEvent('wickx-cityhall:server:getIDs')
end)

RegisterNetEvent('wickx-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['wickx-core']:DrawText('[E] Open Cityhall') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('wickx-cityhall:server:requestId', id.type, closestCityhall)
        WKXCore.Functions.Notify(('You have received your %s for $%s'):format(license.label, id.cost), 'success', 3500)
    else
        WKXCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('wickx-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        WKXCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
