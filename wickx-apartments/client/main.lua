...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and not InApartment then
            exports['wickx-core']:DrawText(Lang:t('text.options'), 'left')
        else
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports['wickx-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and text then
            exports['wickx-core']:DrawText(text, 'left')
        else
            exports['wickx-core']:HideText()
        end
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    exports['wickx-target']:AddBoxZone(boxName, coords, 1.5, 1.5, {
        name = boxName,
        heading = heading,
        minZ = coords.z - 1.0,
        maxZ = coords.z + 5.0,
...
local function DeleteApartmentsEntranceTargets()
    if Apartments.Locations and next(Apartments.Locations) then
        for id, apartment in pairs(Apartments.Locations) do
            if UseTarget then
                exports['wickx-target']:RemoveZone('apartmentEntrance_' .. id)
            else
...
local function DeleteInApartmentTargets()
    ...
    if InApartmentTargets and next(InApartmentTargets) then
        for id, apartmentTarget in pairs(InApartmentTargets) do
            if UseTarget then
                exports['wickx-target']:RemoveZone('inApartmentTarget_' .. id)
            else
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    WKXCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            WKXCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['wickx-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('wickx-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('wickx-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('wickx-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('wickx-interior:client:SetNewState', true)
            else
                TriggerEvent('wickx-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('wickx-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('wickx-apartments:returnBucket')
    DoScreenFadeOut(500)
    ...
    exports['wickx-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('wickx-apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        WKXCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    WKXCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            WKXCore.Functions.Notify(Lang:t('error.nobody_home'), 'error', 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['wickx-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    WKXCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('wickx-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('wickx-spawn:client:openUI', true)
            TriggerEvent('apartments:client:SetHomeBlip', result.type)
        else
            if Apartments.Starting then
                TriggerEvent('wickx-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('wickx-spawn:client:openUI', true)
            else
                TriggerEvent('wickx-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('wickx-spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            WKXCore.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('wickx-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
    WKXCore.Functions.Notify(Lang:t('info.at_the_door'))
end)
