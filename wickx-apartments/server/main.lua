...
RegisterNetEvent('wickx-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        WKXCore.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData('inside', insideMeta)
        WKXCore.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('wickx-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    local src = source
    exports['wickx-inventory']:OpenInventory(src, CurrentApartment)
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label, firstSpawn)
    ...
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.receive_apart') .. ' (' .. label .. ')')
    if firstSpawn then
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent('apartments:client:SetHomeBlip', src, type)
end)
...
WKXCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
WKXCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
WKXCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
WKXCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
WKXCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
WKXCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
