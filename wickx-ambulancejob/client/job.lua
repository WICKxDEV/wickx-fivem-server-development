...
function MenuGarage()
    ...
    local authorizedVehicles = getAuthorizedVehicles(WKXCore.Functions.GetPlayerData().job.grade.level)
...
RegisterNetEvent('WKXCore:Client:SetDuty', function(duty)
...
function Status()
    if isStatusChecking then
        ...
        exports['wickx-menu']:openMenu(statusMenu)
    end
end
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        WKXCore.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result['WEAPONWOUNDS'] then
                        for _, v2 in pairs(result['WEAPONWOUNDS']) do
                            TriggerEvent('chat:addMessage', {
                                color = { 255, 0, 0 },
                                multiline = false,
                                args = { Lang:t('info.status'), WKXCore.Shared.Weapons[v2].damagereason }
                            })
                        end
                    elseif result['BLEED'] > 0 then
                        ...
                        WKXCore.Functions.Notify(Lang:t('success.healthy_player'), 'success')
                    end
                end
        ...
        WKXCore.Functions.Notify(Lang:t('error.no_player'), 'error')
    end
end)

RegisterNetEvent('hospital:client:RevivePlayer', function()
    local hasItem = WKXCore.Functions.HasItem('firstaid')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            WKXCore.Functions.Progressbar('hospital_revive', Lang:t('progress.revive'), 5000, false, true, {
                ...
                WKXCore.Functions.Notify(Lang:t('success.revived'), 'success')
        ...
        WKXCore.Functions.Notify(Lang:t('error.no_firstaid'), 'error')
    end
end)

RegisterNetEvent('hospital:client:TreatWounds', function()
    local hasItem = WKXCore.Functions.HasItem('bandage')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            WKXCore.Functions.Progressbar('hospital_healwounds', Lang:t('progress.healing'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                StopAnimTask(PlayerPedId(), healAnimDict, 'exit', 1.0)
                WKXCore.Functions.Notify(Lang:t('success.helped_player'), 'success')
        ...
        WKXCore.Functions.Notify(Lang:t('error.no_bandage'), 'error')
    end
end)
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                exports['wickx-core']:KeyPressed(38)
                if variable == 'sign' then
                    TriggerEvent('EMSToggle:Duty')
                elseif variable == 'stash' then
                    TriggerServerEvent('wickx-ambulancejob:server:stash')
                elseif variable == 'storeheli' then
                    TriggerEvent('wickx-ambulancejob:storeheli')
                elseif variable == 'takeheli' then
                    TriggerEvent('wickx-ambulancejob:pullheli')
                elseif variable == 'roof' then
                    TriggerEvent('wickx-ambulancejob:elevator_main')
                elseif variable == 'main' then
                    TriggerEvent('wickx-ambulancejob:elevator_roof')
                end
            end
            Wait(1)
        end
        ...
        while CheckVehicle do
            if IsControlJustPressed(0, 38) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    WKXCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                else
...
RegisterNetEvent('wickx-ambulancejob:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('wickx-ambulancejob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for i = 1, #Config.Locations['vehicle'] do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' and onDuty then
                exports['wickx-core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(i)
            else
                CheckVehicle = false
    ...
    for i = 1, #Config.Locations['helicopter'] do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' and onDuty then
                exports['wickx-core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(i)
            else
                CheckHeli = false
                exports['wickx-core']:HideText()
...
if Config.UseTarget then
    CreateThread(function()
        for i = 1, #Config.Locations['duty'] do
            ...
            exports['wickx-target']:AddBoxZone('duty' .. i, vector3(v.x, v.y, v.z), 1.5, 1, {
                name = 'duty' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for i = 1, #Config.Locations['stash'] do
            ...
            exports['wickx-target']:AddBoxZone('stash' .. i, vector3(v.x, v.y, v.z), 1, 1, {
                name = 'stash' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'server',
                        event = 'wickx-ambulancejob:server:stash',
                        icon = 'fa fa-hand',
                        label = 'Open Stash',
                        job = 'ambulance'
                    }
        ...
        for i = 1, #Config.Locations['roof'] do
            ...
            exports['wickx-target']:AddBoxZone('roof' .. i, vector3(v.x, v.y, v.z), 2, 2, {
                name = 'roof' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'wickx-ambulancejob:elevator_roof',
        ...
        for i = 1, #Config.Locations['main'] do
            ...
            exports['wickx-target']:AddBoxZone('main' .. i, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = 'main' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'wickx-ambulancejob:elevator_main',
                        icon = 'fas fa-hand-point-up',
                        label = 'Take Elevator',
                        job = 'ambulance'
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if not onDuty then
                    exports['wickx-core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls('sign')
                else
                    exports['wickx-core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls('sign')
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['wickx-core']:DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls('stash')
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['wickx-core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls('main')
                else
                    exports['wickx-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                exports['wickx-core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['wickx-core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls('roof')
                else
                    exports['wickx-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
