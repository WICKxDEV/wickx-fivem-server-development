...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            WKXCore.Functions.Notify(limbDamageMsg, 'primary')
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        WKXCore.Functions.Notify(Lang:t('info.bleed_alert', { bleedstate = Config.BleedingStates[tonumber(isBleeding)].label }), 'error')
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(WKXCore.Shared.Weapons) do
        if HasPedBeenDamagedByWeapon(ped, k, 0) then
            detected = true
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    WKXCore.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    WKXCore.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent('hospital:server:RestoreWeaponDamage')
    WKXCore.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function()
        Wait(5)
        if isRevive then
            WKXCore.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports['wickx-policejob']:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount, hospitalName)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if WKXCore.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = WKXCore.Functions.GetPlayerData().charinfo
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['wickx-core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == 'checkin' then
                    TriggerEvent('wickx-ambulancejob:checkin')
                    listen = false
                elseif variable == 'beds' then
                    TriggerEvent('wickx-ambulancejob:beds', hospitalIndex, bedId)
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('wickx-ambulancejob:checkin', function()
    ...
    for i = 1, #Config.Locations['hospital'] do
        ...
        if distance < 3 then
            if doctorCount >= Config.MinimalDoctors then
                ...
                WKXCore.Functions.Notify('Called a Doctor', 'primary')
            else
                TriggerEvent('animations:client:EmoteCommandStart', { 'notepad' })
                WKXCore.Functions.Progressbar('hospital_checkin', Lang:t('progress.checking_in'), 2000, false, true, {
                    ...
                    if bedId then
                        ...
                        WKXCore.Functions.Notify(Lang:t('error.beds_taken'), 'error')
                    end
                end)
            end
        end
...
RegisterNetEvent('wickx-ambulancejob:beds', function(hospitalIndex, bedId)
    if bedId then
...
if Config.UseTarget then
    CreateThread(function()
        for i = 1, #Config.Locations['checking'] do
            ...
            exports['wickx-target']:AddBoxZone('checking' .. i, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = 'checking' .. i,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        ...
                        event = 'wickx-ambulancejob:checkin',
                        label = 'Check In',
                    }
                },
                distance = 1.5
        ...
        for hospitalKey = 1, #Config.Locations['hospital'] do
            for bedKey = 1, #Config.Locations['hospital'][hospitalKey]['beds'] do
                ...
                exports['wickx-target']:AddBoxZone('beds' .. bedKey, v.coords, 2.5, 2.3, {
        ...
        for i = 1, #Config.Locations['checking'] do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['wickx-core']:DrawText(Lang:t('text.call_doc'), 'left')
                        CheckInControls('checkin')
                    else
                        exports['wickx-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls('checkin')
                    end
                else
                    listen = false
            ...
            for bedKey = 1, #Config.Locations['hospital'][hospitalKey]['beds'] do
                ...
                bedCombo:onPlayerInOut(function(isPointInside, _, zone)
                    if isPointInside and not isInHospitalBed then
                        exports['wickx-core']:DrawText(Lang:t('text.lie_bed'), 'left')
