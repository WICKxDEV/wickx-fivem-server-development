...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive, hospitalIndex)
	...
	exports['wickx-banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
		...
		for i = 1, #Config.Locations['jailbeds'] do
			...
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney('bank', Config.BillCost, 'respawned-at-hospital')
				exports['wickx-banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	local Patient = WKXCore.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' then
			exports['wickx-inventory']:RemoveItem(src, 'bandage', 1, false, 'hospital:server:TreatWounds')
			TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['bandage'], 'remove')
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' or WKXCore.Functions.HasItem(src, 'firstaid', 1) then
			if oldMan then
				if Player.Functions.RemoveMoney('cash', 5000, 'revived-player') then
					exports['wickx-inventory']:RemoveItem(src, 'firstaid', 1, false, 'hospital:server:RevivePlayer')
					TriggerClientEvent('wickx-inventory:client:ItemBox', src, WKXCore.Shared.Items['firstaid'], 'remove')
			...
			TriggerEvent('wickx-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'wickx-ambulancejob', 'Trying to revive theirselves or other players'), true)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function(hospitalName)
	...
	if not doctorCalled then
		...
		for _, v in pairs(players) do
			if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
				TriggerClientEvent('WKXCore:Notify', v.PlayerData.source, Lang:t('info.dr_needed', { hospital = hospitalName }), 'ambulance')
			end
		end
		...
		TriggerClientEvent('WKXCore:Notify', src, 'Doctor has already been notified', 'error')
	end
end)

RegisterNetEvent('hospital:server:UseFirstAid', function(targetId)
	local src = source
	local Target = WKXCore.Functions.GetPlayer(targetId)
	if Target then
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('WKXCore:Notify', helperId, Lang:t('error.cant_help'), 'error')
	end
end)
...
RegisterNetEvent('hospital:server:removeBandage', function()
	...
	exports['wickx-inventory']:RemoveItem(source, 'bandage', 1, false, 'hospital:server:removeBandage')
end)

RegisterNetEvent('hospital:server:removeIfaks', function()
	local Player = WKXCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['wickx-inventory']:RemoveItem(source, 'ifaks', 1, false, 'hospital:server:removeIfaks')
end)

RegisterNetEvent('hospital:server:removePainkillers', function()
	local Player = WKXCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['wickx-inventory']:RemoveItem(source, 'painkillers', 1, false, 'hospital:server:removePainkillers')
end)
...
RegisterNetEvent('wickx-ambulancejob:server:stash', function()
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if not Player then return end
...
WKXCore.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
	local players = WKXCore.Functions.GetWKXPlayers()
	for _, v in pairs(players) do
		if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
...
WKXCore.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
...
WKXCore.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
WKXCore.Commands.Add('911e', Lang:t('info.ems_report'), { { name = 'message', help = Lang:t('info.message_sent') } }, false, function(source, args)
	local src = source
	local message
...
WKXCore.Commands.Add('status', Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
		TriggerClientEvent('hospital:client:CheckStatus', src)
	else
		TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.not_ems'), 'error')
	end
end)

WKXCore.Commands.Add('heal', Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
WKXCore.Commands.Add('revivep', Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
WKXCore.Commands.Add('revive', Lang:t('info.revive_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		...
		if Player then
			...
			TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.not_online'), 'error')
		end
	else
...
WKXCore.Commands.Add('setpain', Lang:t('info.pain_level'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = WKXCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
WKXCore.Commands.Add('kill', Lang:t('info.kill'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = WKXCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
WKXCore.Commands.Add('aheal', Lang:t('info.heal_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = WKXCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
WKXCore.Functions.CreateUseableItem('ifaks', function(source, item)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
...
WKXCore.Functions.CreateUseableItem('bandage', function(source, item)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
WKXCore.Functions.CreateUseableItem('painkillers', function(source, item)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
WKXCore.Functions.CreateUseableItem('firstaid', function(source, item)
	local src = source
	local Player = WKXCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
