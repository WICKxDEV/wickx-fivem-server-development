...
PlayerData = WKXCore.Functions.GetPlayerData() -- Setting this for when you restart the resource in game
    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items + 1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'wickx-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    ...
                    icon = 'caret-up',
                    type = 'client',
                    event = 'wickx-radialmenu:client:ChangeSeat',
                    shouldClose = false,
                }
            end
        end
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle, 1 do
        if bool then
            exports['wickx-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['wickx-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
local function setRadialState(bool, sendMessage, delay)
    ...
    if Config.UseWhilstWalking then
        if bool then
            TriggerEvent('wickx-radialmenu:client:onRadialmenuOpen')
            SetupRadialMenu()
            PlaySoundFrontend(-1, 'NAV', 'HUD_AMMO_SHOP_SOUNDSET', 1)
            controlToggle(true)
        else
            TriggerEvent('wickx-radialmenu:client:onRadialmenuClose')
            controlToggle(false)
        end
...
RegisterNetEvent('wickx-radialmenu:client:noPlayers', function()
    WKXCore.Functions.Notify(Lang:t('error.no_people_nearby'), 'error', 2500)
end)

RegisterNetEvent('wickx-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            local plate = WKXCore.Functions.GetPlate(closestVehicle)
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('wickx-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('wickx-radialmenu:trunk:server:Door', true, plate, door)
                else
        ...
        WKXCore.Functions.Notify(Lang:t('error.no_vehicle_found'), 'error', 2500)
    end
end)

RegisterNetEvent('wickx-radialmenu:client:setExtra', function(data)
    local string = data.id
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    WKXCore.Functions.Notify(Lang:t('error.extra_deactivated', { extra = extra }), 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    WKXCore.Functions.Notify(Lang:t('success.extra_activated', { extra = extra }), 'success', 2500)
                end
            else
                WKXCore.Functions.Notify(Lang:t('error.extra_not_present', { extra = extra }), 'error', 2500)
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.not_driver'), 'error', 2500)
        end
    end
end)

RegisterNetEvent('wickx-radialmenu:trunk:client:Door', function(plate, door, open)
    local veh = GetVehiclePedIsIn(PlayerPedId())
    if veh ~= 0 then
        local pl = WKXCore.Functions.GetPlate(veh)
        if pl == plate then
            if open then
...
RegisterNetEvent('wickx-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
    local IsSeatFree = IsVehicleSeatFree(Veh, data.id)
    local speed = GetEntitySpeed(Veh)
    local HasHarnass = exports['wickx-smallresources']:HasHarness()
    if not HasHarnass then
        local kmh = speed * 3.6
        if IsSeatFree then
            if kmh <= 100.0 then
                SetPedIntoVehicle(PlayerPedId(), Veh, data.id)
                WKXCore.Functions.Notify(Lang:t('info.switched_seats', { seat = data.title }))
            else
                WKXCore.Functions.Notify(Lang:t('error.vehicle_driving_fast'), 'error')
            end
        else
            WKXCore.Functions.Notify(Lang:t('error.seat_occupied'), 'error')
        end
    else
        WKXCore.Functions.Notify(Lang:t('error.race_harness_on'), 'error')
    end
end)

RegisterNetEvent('wickx-radialmenu:flipVehicle', function()
    WKXCore.Functions.Progressbar('pick_grape', Lang:t('progress.flipping_car'), Config.Fliptime, false, true, {
        ...
        WKXCore.Functions.Notify(Lang:t('task.cancel_task'), 'error')
        StopAnimTask(PlayerPedId(), 'mini@repair', 'fixing_a_ped', 1.0)
    end)
end)
...
RegisterNUICallback('selectItem', function(inData, cb)
    ...
    if data and found then
        ...
        elseif data.type == 'wickxcommand' then
            TriggerServerEvent('WKXCore:CallCommand', data.event, data)
        end
    end
    cb('ok')
end)
