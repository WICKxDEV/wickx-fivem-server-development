...
function TakeOutVehicle(vehicleInfo)
    ...
    WKXCore.Functions.TriggerCallback('WKXCore:Server:SpawnVehicle', function(netId)
        ...
        TriggerServerEvent("wickx-newsjob:server:addVehicleItems", WKXCore.Functions.GetPlate(veh))
...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[WKXCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "wickx-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[WKXCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "wickx-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('wickx-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('wickx-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
