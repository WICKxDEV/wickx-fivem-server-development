...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('wickx-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('wickx-admin:client:toggleBlips', function()
    WKXCore.Functions.TriggerCallback('wickx-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        if not ShowBlips then
            ShowBlips = true
            NetCheck1 = true
            WKXCore.Functions.Notify(Lang:t('success.blips_activated'), 'success')
        else
            ShowBlips = false
            WKXCore.Functions.Notify(Lang:t('error.blips_deactivated'), 'error')
        end
    end)
end)

RegisterNetEvent('wickx-admin:client:toggleNames', function()
    WKXCore.Functions.TriggerCallback('wickx-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
        if not ShowNames then
            ShowNames = true
            NetCheck2 = true
            WKXCore.Functions.Notify(Lang:t('success.names_activated'), 'success')
        else
            ShowNames = false
            WKXCore.Functions.Notify(Lang:t('error.names_deactivated'), 'error')
        end
    end)
end)

RegisterNetEvent('wickx-admin:client:Show', function(players)
    WKXCore.Functions.TriggerCallback('wickx-admin:isAdmin', function(isAdmin)
        if not isAdmin then return end
