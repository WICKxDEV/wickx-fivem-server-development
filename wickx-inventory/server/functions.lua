...
function SetInventory(identifier, items, reason)
    local player = WKXCore.Functions.GetPlayer(identifier)
    ...
    if player then
        ...
        if not player.Offline then
            ...
            TriggerEvent('wickx-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', logMessage)
        end
    ...
    local resourceName = GetInvokingResource() or 'wickx-inventory'
    TriggerEvent(
        'wickx-log:server:CreateLog',
        'playerinventory',
        'Inventory Set',
        'blue',
        '**Inventory:** ' .. invName .. '\n' ..
...
function UseItem(itemName, ...)
    local itemData = WKXCore.Functions.CanUseItem(itemName)
...
function CanAddItem(identifier, item, amount)
    ...
    local itemData = WKXCore.Shared.Items[item:lower()]
...
function ClearInventory(source, filterItems)
    ...
    if not player.Offline then
        ...
        TriggerEvent('wickx-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', logMessage)
...
-- Finally, it triggers the 'wickx-inventory:client:closeInv' event for the given source.
function CloseInventory(source, identifier)
    ...
    TriggerClientEvent('wickx-inventory:client:closeInv', source)
end
...
function OpenInventoryById(source, targetId)
    ...
    local TargetPlayer = WKXCore.Functions.GetPlayer(tonumber(targetId))
    if not WKXPlayer or not TargetPlayer then return end
    if Player(targetId).state.inv_busy then CloseInventory(targetId) end
    local playerItems = WKXPlayer.PlayerData.items
    ...
    TriggerClientEvent('wickx-inventory:client:openInventory', source, playerItems, formattedInventory)
end
...
function OpenInventory(source, identifier, data)
    ...
    if not identifier then
        ...
        TriggerClientEvent('wickx-inventory:client:openInventory', source, WKXPlayer.PlayerData.items)
        return
    end
    ...
    if inventory and inventory.isOpen then
        TriggerClientEvent('WKXCore:Notify', source, 'This inventory is currently in use', 'error')
        return
    end
    ...
    TriggerClientEvent('wickx-inventory:client:openInventory', source, WKXPlayer.PlayerData.items, formattedInventory)
end
    ...
    if not updated then
        ...
        if itemInfo.type == 'weapon' then
            if not inventory[slot].info.serie then
                inventory[slot].info.serie = tostring(WKXCore.Shared.RandomInt(2) .. WKXCore.Shared.RandomStr(3) .. WKXCore.Shared.RandomInt(1) .. WKXCore.Shared.RandomStr(2) .. WKXCore.Shared.RandomInt(3) .. WKXCore.Shared.RandomStr(4))
            end
...
function RemoveItem(identifier, item, amount, slot, reason)
    if not WKXCore.Shared.Items[item:lower()] then
