...
RegisterNetEvent('WKXCore:Server:UpdateObject', function()
    if source ~= '' then return end
    WKXCore = exports['wickx-core']:GetCoreObject()
end)

AddEventHandler('WKXCore:Server:PlayerLoaded', function(Player)
    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'AddItem', function(item, amount, slot, info, reason)
        return AddItem(Player.PlayerData.source, item, amount, slot, info, reason)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'RemoveItem', function(item, amount, slot, reason)
        return RemoveItem(Player.PlayerData.source, item, amount, slot, reason)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'GetItemBySlot', function(slot)
        return GetItemBySlot(Player.PlayerData.source, slot)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'GetItemByName', function(item)
        return GetItemByName(Player.PlayerData.source, item)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'GetItemsByName', function(item)
        return GetItemsByName(Player.PlayerData.source, item)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'ClearInventory', function(filterItems)
        ClearInventory(Player.PlayerData.source, filterItems)
    end)

    WKXCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'SetInventory', function(items)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    for k in pairs(Players) do
        WKXCore.Functions.AddPlayerMethod(k, 'AddItem', function(item, amount, slot, info)
            return AddItem(k, item, amount, slot, info)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'RemoveItem', function(item, amount, slot)
            return RemoveItem(k, item, amount, slot)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'GetItemBySlot', function(slot)
            return GetItemBySlot(k, slot)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'GetItemByName', function(item)
            return GetItemByName(k, item)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'GetItemsByName', function(item)
            return GetItemsByName(k, item)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'ClearInventory', function(filterItems)
            ClearInventory(k, filterItems)
        end)

        WKXCore.Functions.AddPlayerMethod(k, 'SetInventory', function(items)
...
local function checkWeapon(source, item)
    ...
    local weaponInfo = WKXCore.Shared.Weapons[weapon]
    if weaponInfo and weaponInfo.name == currentWeapon then
        RemoveWeaponFromPed(ped, weapon)
        TriggerClientEvent('wickx-weapons:client:UseWeapon', source, { name = currentWeapon }, false)
    end
end

-- Events

RegisterNetEvent('wickx-inventory:server:openVending', function(data)
    local src = source
...
RegisterNetEvent('wickx-inventory:server:closeInventory', function(inventory)
    local src = source
    local WKXPlayer = WKXCore.Functions.GetPlayer(src)
    ...
    if Drops[inventory] then
        ...
        if #Drops[inventory].items == 0 and not Drops[inventory].isOpen then -- if no listeed items in the drop on close
            TriggerClientEvent('wickx-inventory:client:removeDropTarget', -1, Drops[inventory].entityId)
            Wait(500)
...
RegisterNetEvent('wickx-inventory:server:useItem', function(item)
    local src = source
    local itemData = GetItemBySlot(src, item.slot)
    if not itemData then return end
    local itemInfo = WKXCore.Shared.Items[itemData.name]
    if itemData.type == 'weapon' then
        TriggerClientEvent('wickx-weapons:client:UseWeapon', src, itemData, itemData.info.quality and itemData.info.quality > 0)
        TriggerClientEvent('wickx-inventory:client:ItemBox', src, itemInfo, 'use')
    elseif itemData.name == 'id_card' then
        UseItem(itemData.name, src, itemData)
        TriggerClientEvent('wickx-inventory:client:ItemBox', source, itemInfo, 'use')
...
RegisterNetEvent('wickx-inventory:server:openDrop', function(dropId)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    ...
    drop.isOpen = true
    TriggerClientEvent('wickx-inventory:client:openInventory', source, Player.PlayerData.items, formattedInventory)
end)

RegisterNetEvent('wickx-inventory:server:updateDrop', function(dropId, coords)
    Drops[dropId].coords = coords
end)

RegisterNetEvent('wickx-inventory:server:snowball', function(action)
    if action == 'add' then
        AddItem(source, 'weapon_snowball', 1, false, false, 'wickx-inventory:server:snowball')
    elseif action == 'remove' then
        RemoveItem(source, 'weapon_snowball', 1, false, 'wickx-inventory:server:snowball')
    end
end)

-- Callbacks

WKXCore.Functions.CreateCallback('wickx-inventory:server:GetCurrentDrops', function(_, cb)
    cb(Drops)
end)

WKXCore.Functions.CreateCallback('wickx-inventory:server:createDrop', function(source, cb, item)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then
        cb(false)
    ...
    if RemoveItem(src, item.name, item.amount, item.fromSlot, 'dropped item') then
        ...
        if not Drops[newDropId] then
            ...
            TriggerClientEvent('wickx-inventory:client:setupDropTarget', -1, dropId)
        else
...
WKXCore.Functions.CreateCallback('wickx-inventory:server:attemptPurchase', function(source, cb, data)
    ...
    if amount > shopInfo.items[itemInfo.slot].amount then
        TriggerClientEvent('WKXCore:Notify', source, 'Cannot purchase larger quantity than currently in stock', 'error')
        cb(false)
        return
    end

    if not CanAddItem(source, itemInfo.name, amount) then
        TriggerClientEvent('WKXCore:Notify', source, 'Cannot hold item', 'error')
        cb(false)
        return
    end
    ...
    if Player.PlayerData.money.cash >= price then
        ...
        TriggerEvent('wickx-shops:server:UpdateShopItems', shop, itemInfo, amount)
        cb(true)
    else
        TriggerClientEvent('WKXCore:Notify', source, 'You do not have enough money', 'error')
        cb(false)
    end
end)

WKXCore.Functions.CreateCallback('wickx-inventory:server:giveItem', function(source, cb, target, item, amount, slot, info)
    local player = WKXCore.Functions.GetPlayer(source)
    ...
    local Target = WKXCore.Functions.GetPlayer(target)
    ...
    local itemInfo = WKXCore.Shared.Items[item:lower()]
    ...
    TriggerClientEvent('wickx-inventory:client:giveAnim', source)
    TriggerClientEvent('wickx-inventory:client:ItemBox', source, itemInfo, 'remove', giveAmount)
    TriggerClientEvent('wickx-inventory:client:giveAnim', target)
    TriggerClientEvent('wickx-inventory:client:ItemBox', target, itemInfo, 'add', giveAmount)
    if Player(target).state.inv_busy then TriggerClientEvent('wickx-inventory:client:updateInventory', target) end
    cb(true)
end)
...
local function getItem(inventoryId, src, slot)
    ...
    elseif inventoryId:find('otherplayer-') then
        ...
        local targetPlayer = WKXCore.Functions.GetPlayer(targetId)
...
RegisterNetEvent('wickx-inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
