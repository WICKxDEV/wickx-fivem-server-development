...
local function FormatWeaponAttachments(itemdata)
    ...
    local WeaponAttachments = exports['wickx-weapons']:getConfigWeaponAttachments()
        ...
        if componentHash then
            for _, attachmentData in pairs(itemdata.info.attachments) do
                if attachmentData.component == componentHash then
                    local label = WKXCore.Shared.Items[attachmentType] and WKXCore.Shared.Items[attachmentType].label or 'Unknown'
...
RegisterNetEvent('wickx-inventory:client:requiredItems', function(items, bool)
    local itemTable = {}
    if bool then
        for k in pairs(items) do
            itemTable[#itemTable + 1] = {
                item = items[k].name,
                label = WKXCore.Shared.Items[items[k].name]['label'],
                image = items[k].image,
            }
        end
    end
...
RegisterNetEvent('wickx-inventory:client:hotbar', function(items)
...
RegisterNetEvent('wickx-inventory:client:closeInv', function()
    SendNUIMessage({
        action = 'close',
    })
end)

RegisterNetEvent('wickx-inventory:client:updateInventory', function()
    local items = {}
    if PlayerData and type(PlayerData.items) == "table" then
...
RegisterNetEvent('wickx-inventory:client:ItemBox', function(itemData, type, amount)
    SendNUIMessage({
        action = 'itemBox',
        item = itemData,
        type = type,
...
RegisterNetEvent('wickx-inventory:server:RobPlayer', function(TargetId)
    SendNUIMessage({
        action = 'RobMoney',
        TargetId = TargetId,
    })
...
RegisterNetEvent('wickx-inventory:client:openInventory', function(items, other)
...
RegisterNetEvent('wickx-inventory:client:giveAnim', function()
...
RegisterNUICallback('AttemptPurchase', function(data, cb)
    WKXCore.Functions.TriggerCallback('wickx-inventory:server:attemptPurchase', function(canPurchase)
        cb(canPurchase)
    end, data)
end)
...
RegisterNUICallback('CloseInventory', function(data, cb)
    ...
    if data.name then
        ...
        TriggerServerEvent('wickx-inventory:server:closeInventory', data.name)
    elseif CurrentDrop then
        TriggerServerEvent('wickx-inventory:server:closeInventory', CurrentDrop)
        CurrentDrop = nil
    end
    cb('ok')
end)
...
RegisterNUICallback('UseItem', function(data, cb)
    TriggerServerEvent('wickx-inventory:server:useItem', data.item)
    cb('ok')
end)

RegisterNUICallback('SetInventoryData', function(data, cb)
    TriggerServerEvent('wickx-inventory:server:SetInventoryData', data.fromInventory, data.toInventory, data.fromSlot, data.toSlot, data.fromAmount, data.toAmount)
    cb('ok')
end)

RegisterNUICallback('GiveItem', function(data, cb)
    local player, distance = WKXCore.Functions.GetClosestPlayer(GetEntityCoords(PlayerPedId()))
    if player ~= -1 and distance < 3 then
        local playerId = GetPlayerServerId(player)
        WKXCore.Functions.TriggerCallback('wickx-inventory:server:giveItem', function(success)
            cb(success)
        end, playerId, data.item.name, data.amount, data.slot, data.info)
    else
        WKXCore.Functions.Notify(Lang:t('notify.nonb'), 'error')
        cb(false)
    end
end)
...
RegisterNUICallback('GetWeaponData', function(cData, cb)
    local data = {
        WeaponData = WKXCore.Shared.Items[cData.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    local allAttachments = exports['wickx-weapons']:getConfigWeaponAttachments()
    local Attachment = allAttachments[data.AttachmentData.attachment][WeaponData.name]
    local itemInfo = WKXCore.Shared.Items[data.AttachmentData.attachment]
    WKXCore.Functions.TriggerCallback('wickx-weapons:server:RemoveAttachment', function(NewAttachments)
...
CreateThread(function()
    exports['wickx-target']:AddTargetModel(Config.VendingObjects, {
        options = {
            {
                type = 'server',
                event = 'wickx-inventory:server:openVending',
...
for i = 1, 5 do
    RegisterCommand('slot_' .. i, function()
        ...
        if itemData.type == "weapon" then
            if HoldingDrop then
                return WKXCore.Functions.Notify("Your already holding a bag, Go Drop it!", "error", 5500)
            end
        end
        TriggerServerEvent('wickx-inventory:server:useItem', itemData)
    end, false)
