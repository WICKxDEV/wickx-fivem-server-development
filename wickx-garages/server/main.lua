...
local function filterVehiclesByCategory(vehicles, category)
    ...
    for _, vehicle in pairs(vehicles) do
        local vehicleData = WKXCore.Shared.Vehicles[vehicle.vehicle]
...
WKXCore.Functions.CreateCallback('wickx-garages:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

WKXCore.Functions.CreateCallback('wickx-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
local function GetVehicleTypeByModel(model)
    local vehicleData = WKXCore.Shared.Vehicles[model]
...
WKXCore.Functions.CreateCallback('wickx-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    local vehType = WKXCore.Shared.Vehicles[vehicle] and WKXCore.Shared.Vehicles[vehicle].type or GetVehicleTypeByModel(vehicle)
...
WKXCore.Functions.CreateCallback('wickx-garages:server:IsSpawnOk', function(_, cb, plate, type)
...
WKXCore.Functions.CreateCallback('wickx-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = WKXCore.Functions.GetPlayer(source)
    ...
    if type == 'house' and not exports['wickx-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then
        cb(false)
        return
    end
    if state == 1 then
...
RegisterNetEvent('wickx-garages:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('wickx-garages:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('wickx-garages:server:UpdateOutsideVehicle', function(plate, vehicleNetID)
...
RegisterNetEvent('wickx-garages:server:trackVehicle', function(plate)
    local src = source
    local vehicleData = OutsideVehicles[plate]
    if vehicleData and DoesEntityExist(vehicleData.entity) then
        TriggerClientEvent('wickx-garages:client:trackVehicle', src, GetEntityCoords(vehicleData.entity))
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('success.vehicle_tracked'), 'success')
    else
        TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.vehicle_not_tracked'), 'error')
    end
end)

RegisterNetEvent('wickx-garages:server:PayDepotPrice', function(data)
    local src = source
    local Player = WKXCore.Functions.GetPlayer(src)
    ...
    MySQL.scalar('SELECT depotprice FROM player_vehicles WHERE plate = ?', { data.plate }, function(result)
        if result then
            ...
            if cashBalance >= depotPrice then
                ...
                TriggerClientEvent('wickx-garages:client:takeOutGarage', src, data)
            elseif bankBalance >= depotPrice then
                Player.Functions.RemoveMoney('bank', depotPrice, 'paid-depot')
                TriggerClientEvent('wickx-garages:client:takeOutGarage', src, data)
            else
                TriggerClientEvent('WKXCore:Notify', src, Lang:t('error.not_enough'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('wickx-garages:server:syncGarage', function(updatedGarages)
    Config.Garages = updatedGarages
end)

--Call from wickx-phone

WKXCore.Functions.CreateCallback('wickx-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = WKXCore.Functions.GetPlayer(source)
    local Vehicles = {}
    ...
    MySQL.rawExecute('SELECT * FROM player_vehicles WHERE citizenid = ?', { Player.PlayerData.citizenid }, function(result)
        if result[1] then
            for _, v in pairs(result) do
                local VehicleData = WKXCore.Shared.Vehicles[v.vehicle]
