...
local function OpenGarageMenu(data)
    WKXCore.Functions.TriggerCallback('wickx-garages:server:GetGarageVehicles', function(result)
        if result == nil then return WKXCore.Functions.Notify(Lang:t('error.no_vehicles'), 'error', 5000) end
        local formattedVehicles = {}
        for _, v in pairs(result) do
            local enginePercent = WKXCore.Shared.Round(v.engine, 0)
            local bodyPercent = WKXCore.Shared.Round(v.body, 0)
            local vname = nil
            pcall(function()
                vname = WKXCore.Shared.Vehicles[v.vehicle].name
            end)
...
local function DepositVehicle(veh, data)
    ...
    WKXCore.Functions.TriggerCallback('wickx-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            ...
            TriggerServerEvent('wickx-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('wickx-garages:server:UpdateOutsideVehicle', plate, nil) end
            WKXCore.Functions.Notify(Lang:t('success.vehicle_parked'), 'primary', 4500)
        else
            WKXCore.Functions.Notify(Lang:t('error.not_owned'), 'error', 3500)
        end
...
local function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            ...
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
                                WKXCore.Functions.Notify(Lang:t('error.not_correct_type'), 'error', 3500)
                                return
                            end
            ...
            exports['wickx-core']:DrawText(displayText, 'left')
        else
            listenForKey = false
...
function GetSpawnPoint(garage)
    ...
    if not location then
        WKXCore.Functions.Notify(Lang:t('error.vehicle_occupied'), 'error')
    end
    return location
end
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('wickx-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('wickx-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('wickx-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('wickx-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('wickx-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('wickx-garages:client:takeOutGarage', function(data)
    WKXCore.Functions.TriggerCallback('wickx-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            WKXCore.Functions.TriggerCallback('wickx-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                while not NetworkDoesNetworkIdExist(netId) do Wait(10) end
                local veh = NetworkGetEntityFromNetworkId(netId)
                Citizen.Await(CheckPlate(veh, vehPlate))
                WKXCore.Functions.SetVehicleProperties(veh, properties)
                exports[Config.FuelResource]:SetFuel(veh, data.stats.fuel)
                TriggerServerEvent('wickx-garages:server:updateVehicleState', 0, vehPlate)
            ...
            WKXCore.Functions.Notify(Lang:t('error.not_depot'), 'error', 5000)
        end
...
local function CreateHouseZone(index, garage, zoneType)
    ...
    houseComboZones:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            exports['wickx-core']:DrawText(Lang:t('info.house_garage'), 'left')
        else
...
RegisterNetEvent('wickx-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        WKXCore.Functions.TriggerCallback('wickx-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('wickx-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('wickx-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('wickx-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
...
RegisterNetEvent('WKXCore:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('WKXCore:Client:OnJobUpdate', function(job)
    PlayerJob = job
end)
