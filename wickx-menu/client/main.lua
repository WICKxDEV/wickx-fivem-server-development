...
RegisterNetEvent('WKXCore:Client:UpdateObject', function() WKXCore = exports['wickx-core']:GetCoreObject() end)
...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			if WKXCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(WKXCore.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://wickx-inventory/html/images/"..WKXCore.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
...
RegisterNetEvent('wickx-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('wickx-menu:client:closeMenu', function()
    closeMenu()
end)
    ...
    if sendData then
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isWKXCommand then
                    TriggerServerEvent('WKXCore:CallCommand', data.params.event, data.params.args)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("wickx-menu:client:menuClosed")
end)
