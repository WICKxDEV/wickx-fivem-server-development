...
function WKXCore.Functions.CreateClientCallback(name, cb)
    WKXCore.ClientCallbacks[name] = cb
end

function WKXCore.Functions.TriggerCallback(name, ...)
    local cb = nil
    local args = { ... }
    ...
    WKXCore.ServerCallbacks[name] = {
        callback = cb,
        promise = promise.new()
    }

    TriggerServerEvent('WKXCore:Server:TriggerCallback', name, table.unpack(args))

    if cb == nil then
        Citizen.Await(WKXCore.ServerCallbacks[name].promise)
        return WKXCore.ServerCallbacks[name].promise.value
    end
end

function WKXCore.Debug(resource, obj, depth)
    TriggerServerEvent('WKXCore:DebugSomething', resource, obj, depth)
end

-- Player

function WKXCore.Functions.GetPlayerData(cb)
    if not cb then return WKXCore.PlayerData end
    cb(WKXCore.PlayerData)
end
...
function WKXCore.Functions.HasItem(items, amount)
    return exports['wickxinventory']:HasItem(items, amount)
end

---Returns the full character name
---@return string
function WKXCore.Functions.GetName()
    local charinfo = WKXCore.PlayerData.charinfo
...
function WKXCore.Functions.LookAtEntity(entity, timeout, speed)
...
function WKXCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function WKXCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if WKXCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if WKXCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function WKXCore.Functions.Notify(text, texttype, length, icon)
    local message = {
        action = 'notify',
...
function WKXCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for WKXCore.Functions.Progressbar to work') end
...
function WKXCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function WKXCore.Functions.GetObjects()
...
function WKXCore.Functions.GetPlayersFromCoords(coords, distance)
...
function WKXCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = WKXCore.Functions.GetPlayersFromCoords(coords)
...
function WKXCore.Functions.GetPeds(ignoreList)
...
function WKXCore.Functions.GetClosestPed(coords, ignoreList)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    ignoreList = ignoreList or {}
    local peds = WKXCore.Functions.GetPeds(ignoreList)
    local closestDistance = -1
    local closestPed = -1
    for i = 1, #peds, 1 do
...
function WKXCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function WKXCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function WKXCore.Functions.LoadModel(model)
...
function WKXCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    WKXCore.Functions.LoadModel(model)
...
function WKXCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function WKXCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return WKXCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function WKXCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function WKXCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = WKXCore.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = WKXCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = WKXCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = WKXCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = WKXCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = WKXCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = WKXCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function WKXCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function WKXCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function WKXCore.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function WKXCore.Functions.RequestAnimDict(animDict)
...
function WKXCore.Functions.GetClosestBone(entity, list)
...
function WKXCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function WKXCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and joaat(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    WKXCore.Functions.LoadModel(modelHash)
...
function WKXCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function WKXCore.Functions.LoadAnimSet(animSet)
...
function WKXCore.Functions.LoadParticleDictionary(dictionary)
...
function WKXCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    WKXCore.Functions.LoadParticleDictionary(dict)
...
function WKXCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    WKXCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function WKXCore.Functions.GetStreetNametAtCoords(coords)
...
function WKXCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function WKXCore.Functions.GetCardinalDirection(entity)
...
function WKXCore.Functions.GetCurrentTime()
    local obj = {}
...
function WKXCore.Functions.GetGroundZCoord(coords)
...
function WKXCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
