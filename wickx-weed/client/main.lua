    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            local plantData = {
                ...
                ['plantProp'] = joaat(WKXWeed.Plants[housePlants[currentHouse][k].sort].stages[housePlants[currentHouse][k].stage]),
            }
...
local function updateHousePlants(leftHouse)
    ...
    CreateThread(function()
        for k in pairs(housePlants[currentHouse]) do
            ...
            for _, stage in pairs(WKXWeed.Plants[housePlants[currentHouse][k].sort]['stages']) do
        ...
        if leftHouse then
            ...
            WKXCore.Functions.TriggerCallback('wickx-weed:server:getBuildingPlants', function(plants)
...
local function getPlantData()
    return {
        ...
        ['plantProp'] = GetHashKey(WKXWeed.Plants[housePlants[currentHouse][closestPlant].sort]['stages'][housePlants[currentHouse][closestPlant].stage]),
        ['plantSort'] = {
            ['name'] = housePlants[currentHouse][closestPlant].sort,
            ['label'] = WKXWeed.Plants[housePlants[currentHouse][closestPlant].sort]['label'],
        },
        ['plantStats'] = {
            ...
            ['highestStage'] = WKXWeed.Plants[housePlants[currentHouse][closestPlant].sort]['highestStage'],
    ...
    CreateThread(function()
        while currentHouse do
            ...
            if plantSpawned and closestPlant ~= 0 then
                ...
                local plantStageLabel = Lang:t('text.stage') .. ' ~b~' .. WKXWeed.StageLabels[plantData['plantStats']['stage']] .. ' ~w~' .. Lang:t('text.progress') .. ' ~b~' .. plantData['plantStats']['progress'] .. '% ~w~ / ' .. Lang:t('text.highestStage') .. ' ~b~' .. WKXWeed.StageLabels[plantData['plantStats']['highestStage']]
                ...
                if plantStageLabel and WKXWeed.ShowStages then drawText3Ds(plantData['plantCoords'].x, plantData['plantCoords'].y, plantData['plantCoords'].z - 0.1, plantStageLabel) end
                ...
                if status > 0 and IsControlJustPressed(0, 38) then
                    ...
                    WKXCore.Functions.Progressbar('remove_weed_plant', Lang:t((status == 1) and 'text.harvesting_plant' or 'text.removing_the_plant'), 8000, false, true, {
                        ...
                        if status == 1 then
                            ...
                            TriggerServerEvent('wickx-weed:server:harvestPlant', currentHouse, amount, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
                        else
                            TriggerServerEvent('wickx-weed:server:removeDeathPlant', currentHouse, plantData['plantStats']['plantId'])
                        end
                    end, function() -- Cancel
                        ClearPedTasks(ped)
                        WKXCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
                    end)
                end
            else
                Wait(100)
...
RegisterNetEvent('wickx-weed:client:getHousePlants', function(house)
    WKXCore.Functions.TriggerCallback('wickx-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        housePlants[currentHouse] = plants
        spawnHousePlants()
...
RegisterNetEvent('wickx-weed:client:leaveHouse', function()
    updateHousePlants(true)
end)

RegisterNetEvent('wickx-weed:client:refreshHousePlants', function(house)
    if currentHouse == house or not house then
        updateHousePlants(false)
    end
end)

RegisterNetEvent('wickx-weed:client:placePlant', function(type, item)
    if not currentHouse then
        WKXCore.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
        return
    end
    if closestPlant ~= 0 then
        WKXCore.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        return
    end
    local ped = PlayerPedId()
    ...
    local plantData = {
        ...
        ['plantModel'] = WKXWeed.Plants[type]['stages']['stage-a'],
        ['plantLabel'] = WKXWeed.Plants[type]['label']
    }

    LocalPlayer.state:set('inv_busy', true, true)
    WKXCore.Functions.Progressbar('plant_weed_plant', Lang:t('text.planting'), 8000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('wickx-weed:server:placePlant', json.encode(plantData['plantCoords']), type, currentHouse)
        TriggerServerEvent('wickx-weed:server:removeSeed', item.slot, type)
    end, function() -- Cancel
        ClearPedTasks(ped)
        WKXCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        LocalPlayer.state:set('inv_busy', false, true)
...
RegisterNetEvent('wickx-weed:client:foodPlant', function()
    ...
    if closestPlant ~= 0 then
        ...
        if plantData['plantStats']['food'] == 100 then
            WKXCore.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
        else
            LocalPlayer.state:set('inv_busy', true, true)
            WKXCore.Functions.Progressbar('plant_weed_plant', Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                TriggerServerEvent('wickx-weed:server:foodPlant', currentHouse, newFood, plantData['plantSort']['name'], plantData['plantStats']['plantId'])
            end, function() -- Cancel
                ClearPedTasks(ped)
                LocalPlayer.state:set('inv_busy', false, true)
                WKXCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
        ...
        WKXCore.Functions.Notify(Lang:t('error.cant_place_here'), 'error')
    end
end)
