...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["wickx-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        WKXCore.Functions.Notify(Lang:t('success.dropped_off'), 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('wickx-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "wickx-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("wickx-busjob:client:TakeVehicle", function(data)
    local coords = Config.Location
    if (BusData.Active) then
        WKXCore.Functions.Notify(Lang:t('error.one_bus_active'), 'error')
        return
    else
...
RegisterNetEvent('wickx-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            WKXCore.Functions.Notify(Lang:t('info.goto_busstop'), 'primary')
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('wickx-busjob:server:NpcPay')
                                exports["wickx-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            WKXCore.Functions.Notify(Lang:t('error.already_driving_bus'), 'error')
        end
    else
        WKXCore.Functions.Notify(Lang:t('error.not_in_bus'), 'error')
    end
end)
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["wickx-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["wickx-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false) then
                                    ...
                                    WKXCore.Functions.Notify(Lang:t('error.drop_off_passengers'), 'error')
                                end
                            end
                        end
                    until not inRange
